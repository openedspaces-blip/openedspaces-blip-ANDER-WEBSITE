...
-- (xp, level, badges, longest_streak, access_tier) added above. Must run
-- after the ALTER TABLE above - the columns referenced below don't exist
-- until it has applied.
--
-- No backfill is needed for existing rows: the ALTER TABLE above added
-- these columns with constant DEFAULTs, which Postgres applies to all
