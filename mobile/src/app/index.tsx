...
function Nav({ label, active, onPress }: { label: string; active?: boolean; onPress: () => void }) { return <Pressable onPress={onPress} style={s.navItem}><ThemedText style={[s.navLabel, active && s.navActive]}>{label}</ThemedText></Pressable>; }
function PlatformTile({ number, title, copy, color, destination }: typeof PLATFORM_TILES[number]) { const tone = color === 'blue' ? s.tileBlue : color === 'violet' ? s.tileViolet : color === 'cyan' ? s.tileCyan : color === 'emerald' ? s.tileEmerald : color === 'orange' ? s.tileOrange : color === 'indigo' ? s.tileIndigo : color === 'rose' ? s.tileRose : s.tileSlate; return <Pressable onPress={() => router.push(destination as never)} style={[s.tile, tone]}><ThemedText style={s.tileNumber}>{number}</ThemedText><ThemedText style={s.tileTitle}>{title}</ThemedText><ThemedText style={s.tileCopy}>{copy}</ThemedText><ThemedText style={s.tileArrow}>→</ThemedText></Pressable>; }

const s = StyleSheet.create({
  safe:{flex:1,backgroundColor:'#F6F8FD'},screen:{flex:1},content:{padding:20,paddingBottom:30,gap:14},topbar:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:2},brandLockup:{flex:1,flexDirection:'row',alignItems:'center',gap:9,minWidth:0},brandMark:{width:36,height:36,borderRadius:12,alignItems:'center',justifyContent:'center',backgroundColor:'#173F91',shadowColor:'#173F91',shadowOpacity:.16,shadowRadius:8,shadowOffset:{width:0,height:3},elevation:3},brandMarkText:{color:'#FFF',fontSize:21,fontWeight:'900',fontStyle:'italic'},brand:{color:'#172554',fontSize:15,fontWeight:'900',letterSpacing:1.4},subtitle:{color:'#64748B',fontSize:10,fontWeight:'700',marginTop:1},accountButton:{maxWidth:130,flexDirection:'row',alignItems:'center',gap:7,paddingHorizontal:8,paddingVertical:6,borderRadius:16,backgroundColor:'#FFF',borderWidth:1,borderColor:'#DCE6F7'},accountAvatar:{fontSize:23},accountInfo:{flexShrink:1},accountText:{color:'#1D4ED8',fontSize:12,fontWeight:'900'},accountPlan:{color:'#64748B',fontSize:9,fontWeight:'800'},
  hero:{overflow:'hidden',borderRadius:26,padding:23,backgroundColor:'#173F91',minHeight:246},heroGlow:{position:'absolute',height:205,width:205,borderRadius:110,backgroundColor:'#0EA5E9',opacity:.44,right:-76,top:-65},heroKicker:{color:'#BFDBFE',fontSize:10,fontWeight:'900',letterSpacing:.6},heroTitle:{color:'#FFF',fontSize:29,lineHeight:34,fontWeight:'900',marginTop:13},heroCopy:{color:'#DBEAFE',fontSize:14,lineHeight:20,marginTop:9,maxWidth:'84%'},heroAction:{alignSelf:'flex-start',marginTop:18,paddingHorizontal:15,paddingVertical:12,borderRadius:14,backgroundColor:'#FFF'},heroActionText:{color:'#1D4ED8',fontSize:13,fontWeight:'900'},
  sectionHead:{flexDirection:'row',alignItems:'baseline',justifyContent:'space-between',marginTop:5},sectionTitle:{color:'#172554',fontSize:18,fontWeight:'900',marginTop:5},sectionHint:{color:'#8190A7',fontSize:10,fontWeight:'700'},languages:{gap:9,paddingRight:16},language:{width:88,minHeight:86,borderRadius:18,padding:10,backgroundColor:'#FFF',borderWidth:1,borderColor:'#E0E7F1',justifyContent:'center'},languageActive:{backgroundColor:'#E8F0FF',borderColor:'#2563EB'},flag:{fontSize:20},languageText:{color:'#475569',fontSize:12,fontWeight:'900',marginTop:6},languageTextActive:{color:'#1D4ED8'},languageLevel:{color:'#94A3B8',fontSize:10,fontWeight:'700',marginTop:2},languageLevelActive:{color:'#3B82F6'},
  platformHead:{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-end',marginTop:5},platformHint:{color:'#64748B',fontSize:11,marginTop:2},platformCount:{fontSize:26,lineHeight:28,fontWeight:'900',color:'#BFDBFE'},tileGrid:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between',rowGap:10},tile:{width:'48%',minHeight:132,borderRadius:20,padding:14,overflow:'hidden'},tileBlue:{backgroundColor:'#1D4ED8'},tileViolet:{backgroundColor:'#6D28D9'},tileCyan:{backgroundColor:'#0891B2'},tileEmerald:{backgroundColor:'#059669'},tileOrange:{backgroundColor:'#D97706'},tileIndigo:{backgroundColor:'#4338CA'},tileRose:{backgroundColor:'#E11D48'},tileSlate:{backgroundColor:'#334155'},tileNumber:{color:'#BFDBFE',fontSize:11,fontWeight:'900',letterSpacing:1},tileTitle:{color:'#FFF',fontSize:19,fontWeight:'900',marginTop:14},tileCopy:{color:'#E0F2FE',fontSize:11,lineHeight:15,marginTop:3,maxWidth:'88%'},tileArrow:{color:'#FFF',fontSize:20,fontWeight:'900',position:'absolute',right:13,bottom:9},bottomNav:{flexDirection:'row',justifyContent:'space-around',paddingTop:13,marginTop:5,borderTopWidth:1,borderColor:'#E2E8F0'},navItem:{minWidth:55,alignItems:'center',paddingVertical:4},navLabel:{color:'#64748B',fontSize:10,fontWeight:'800'},navActive:{color:'#2563EB'},
});
