...
export async function loadCurriculum(
  ...
  try {
    const routeResponse = await fetchWithTimeout(
      `${ANDERGO_API_URL}/api/lessons?language=${language}&level=${level}`,
      {
        headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {}
      }
    );
    const routePayload = (await routeResponse.json().catch(() => null)) as {
    ...
    if (routeResponse.ok && Array.isArray(routePayload?.lessons)) {
      const lessons: CurriculumLesson[] = routePayload.lessons.map((lesson) => ({
        ...
        skill: lesson.skill as CurriculumLesson['skill'],
        title: String(lesson.title ?? ''),
        description: typeof lesson.description === 'string' ? lesson.description : undefined,
        unitId: typeof lesson.unitId === 'string' ? lesson.unitId : undefined,
        unitOrder: typeof lesson.unitOrder === 'number' ? lesson.unitOrder : undefined,
        xpReward: typeof lesson.xpReward === 'number' ? lesson.xpReward : undefined,
        estimated_minutes:
          typeof lesson.estimated_minutes === 'number' ? lesson.estimated_minutes : undefined,
        ...
        bestScore: typeof lesson.bestScore === 'number' ? lesson.bestScore : 0,
        locked: false,
        audioUrl: typeof lesson.audioUrl === 'string' ? lesson.audioUrl : null
      }));
      if (lessons.length) await AsyncStorage.setItem(cacheKey, JSON.stringify(lessons));
      return lessons;
    }
    const select =
      'id,slug,skill,title,description,order_index,xp_reward,access_tier,estimated_minutes,audio_url,unit_id,course_units(slug,order_index),courses!inner(languages!inner(code),levels!inner(code))';
...
export async function loadLessonActivity(
  ...
  // Start audio discovery immediately. It is independent from the lesson body,
  // so the player never adds a second network wait to opening a reading.
  const officialAudioRequest = loadOfficialAudio(slug, context);
  try {
    ...
    const detailResponse = await fetchWithTimeout(
      `${ANDERGO_API_URL}/api/lessons/${encodeURIComponent(slug)}`,
      {
        headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {}
      }
    );
    ...
    if (detailResponse.ok && detail && typeof detail.title === 'string') {
      const readingData = detail.reading as
        { title?: string; text?: string; questions?: string[] } | null | undefined;
      const exercises = Array.isArray(detail.exercises)
        ? (detail.exercises as Record<string, unknown>[])
        : [];
      const quiz = exercises
        ...
        .map((exercise) => ({
          id: String(exercise.id ?? exercise.prompt),
          prompt: String(exercise.prompt),
          options: (exercise.options as Record<string, unknown>[])
            .map((option, index) => ({
      ...
      const vocabulary = Array.isArray(detail.vocabulary)
        ? (detail.vocabulary as { word?: string; translation?: string; example?: string }[])
        : [];
      const dialogue = Array.isArray(detail.dialogue)
        ? (detail.dialogue as { line?: string; translation?: string }[])
        : [];
      const pairs = [
        ...vocabulary.map((item) => ({
          source: item.word?.trim() || '',
          target: item.translation?.trim() || '',
      ...
      const officialAudio = await officialAudioRequest;
      ...
      const fallbackTranscript =
        dialogue
          ...
          .filter(Boolean)
          .join('\n') ||
        readingData?.text?.trim() ||
        null;
      return {
        title: detail.title,
        description: typeof detail.description === 'string' ? detail.description : undefined,
        mission: typeof detail.mission === 'string' ? detail.mission : null,
        grammarNote: typeof detail.grammar === 'string' ? detail.grammar : null,
        audioUrl: typeof detail.audio_url === 'string' ? detail.audio_url : null,
        ...
        pairs,
        reading: readingData?.text
          ? {
              title: readingData.title?.trim() || detail.title,
              text: readingData.text.trim(),
              questions: Array.isArray(readingData.questions)
                ? readingData.questions.filter(Boolean)
                : [],
              quiz
            }
          : null,
    ...
    const select =
      'title,description,mission,grammar_note,audio_url,phrases,lesson_sections(type,word,translation,line,example,reading_title,reading_text,reading_questions,order_index)';
    const response = await fetchWithTimeout(
      `${SUPABASE_URL}/rest/v1/course_lessons?slug=eq.${encodeURIComponent(slug)}&select=${encodeURIComponent(select)}&limit=1`,
      {
        headers: { apikey: SUPABASE_PUBLISHABLE_KEY }
      }
    );
    ...
    const row = (
      (await response.json()) as {
        ...
        audio_url?: string | null;
        phrases?: string[] | null;
        lesson_sections?: LessonSection[] | null;
      }[]
    )[0];
    if (!row) return null;

    const sections = [...(row.lesson_sections ?? [])].sort(
    ...
    const pairs = sections.flatMap((section) => {
      ...
      return source && target
        ? [{ source, target, example: section.example?.trim() || undefined }]
        : [];
    });
    ...
    const fallbackTranscript =
      sections
        ...
        .filter(Boolean)
        .join('\n') ||
      readingSection?.reading_text?.trim() ||
      null;
    ...
    return {
      ...
      reading: readingSection?.reading_text
        ? {
            ...
            questions: Array.isArray(readingSection.reading_questions)
              ? readingSection.reading_questions.filter(Boolean)
              : [],
            quiz: []
          }
        : null,
...
export async function completeMobileLesson(
  slug: string,
  answers: { exerciseId: string; selectedOptionId: string }[],
  accessToken: string
): Promise<MobileCompletion> {
  const response = await fetchWithTimeout(
    `${ANDERGO_API_URL}/api/lessons/${encodeURIComponent(slug)}/complete`,
    {
      method: 'POST',
  ...
  const data = (await response.json().catch(() => ({}))) as Partial<MobileCompletion> & {
    error?: string;
  };
  ...
  return {
    ...
    bestScore: Number(data.bestScore || data.score || 0)
  };
}
...
export async function checkMobileAnswer(
  ...
  const response = await fetchWithTimeout(
    `${ANDERGO_API_URL}/api/lessons/${encodeURIComponent(slug)}/check-answer`,
    {
      method: 'POST',
      headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  ...
  if (!response.ok || typeof data.correct !== 'boolean')
    throw new Error(data.error || 'No se pudo comprobar la respuesta.');
  return {
    correct: data.correct,
    correctOption: typeof data.correctOption === 'number' ? data.correctOption : undefined
  };
}
...
async function loadOfficialAudio(
  ...
  try {
    ...
    // Audio is optional: do not delay a reading more than a few seconds when
    // a legacy lesson has no exported track. Native TTS remains available.
    const response = await fetchWithTimeout(
      `${ANDERGO_API_URL}/api/listening/audio?${audioQuery}`,
      undefined,
      4_500
    );
    ...
    return response.ok && (payload?.status === 'official' || payload?.status === 'partial')
      ? (payload.audio ?? null)
      : null;
  } catch {
    return null;
