...
function buildUnit(language, level, raw, order) {
  ...
  // Reading copy is authored in the target language. These are deliberately
  // complete A1 texts (three short paragraphs), rather than translated or
  // mixed-language placeholders, so learners get enough meaningful context.
  const text = pt
    ? `${c.person} está em ${c.city}. Hoje ela aprende a ${goal}. Ela encontra ${c.friend} perto de um lugar conhecido do bairro. ${c.person} diz: “${phrase}”. ${c.friend} responde com um sorriso e os dois conversam devagar. Eles escutam as pessoas, observam o lugar e repetem frases simples. Durante a conversa, aprendem palavras úteis: ${w1}, ${w2}, ${w3} e ${w4}.`
    : `${c.person} è a ${c.city}. Oggi impara a ${goal}. Incontra ${c.friend} vicino a un luogo conosciuto del quartiere. ${c.person} dice: “${phrase}”. ${c.friend} risponde con un sorriso e i due parlano lentamente. Ascoltano le persone, osservano il luogo e ripetono frasi semplici. Durante la conversazione imparano parole utili: ${w1}, ${w2}, ${w3} e ${w4}.`;
  const readingParts = pt
    ? [
        text,
        `${c.friend} faz uma pergunta simples e ${c.person} responde com uma frase curta. Eles usam as palavras novas em pequenas situações: no café, na escola ou na rua. Assim, cada expressão ganha um sentido claro e fácil de lembrar.`,
        `No final, ${c.person} escreve: “${phrase}”. Ela está contente porque pode participar da conversa e quer praticar de novo amanhã com ${c.friend}.`
      ]
    : [
        text,
  ...
  return { slug, title, titleEs, description: culture, order, accessTier: order <= freeUnitLimit ? 'free' : 'premium', unitOverview: { objective: goal, outcomes: [goal, 'comprender un fragmento cultural breve', 'usar vocabulario en un reto'], grammar: [isA2 ? 'presente y pasado cercano' : 'presente y frases útiles'], vocabulary: [w1,w2,w3,w4], scenario: culture }, activities: {
    reading: base('reading', { title: `${title}: ${pt ? 'leitura' : 'lettura'}`, description: culture, reading: { title, parts: readingParts, questions: pt ? [`Onde está ${c.person}?`, `Com quem ${c.person} conversa?`, `Qual frase ${c.person} usa?`, `Por que a experiência é importante?`] : [`${copy.where} è ${c.person}?`, `${copy.withWho} parla ${c.person}?`, `Quale frase usa ${c.person}?`, `${copy.why} l’esperienza è importante?`] }, exercises: pt ? [mcq('O que a unidade pratica?', goal, ['Uma regra isolada', 'Um exame técnico', 'Um tema sem contexto']), mcq('O que o estudante faz?', goal, ['Memoriza sem usar', 'Evita falar', 'Só traduz'])] : [mcq('Che cosa pratica l’unità?', goal, ['Una regola isolata', 'Un esame tecnico', 'Un tema senza contesto']), mcq('Che cosa fa lo studente?', goal, ['Memorizza senza usare', 'Evita di parlare', 'Traduce soltanto'])] }),
