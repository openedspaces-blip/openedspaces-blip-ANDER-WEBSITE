#!/usr/bin/env node
// scripts/build-french-b1-seed.js
// Folds scripts/content/french-b1-units.js (hand-authored units) into
// lib/seed-lessons.json/lib/seed-units.json, additively - only rows with
// target_language 'french' and level 'B1' are replaced, so this never
// touches the existing French A1/A2 rows. Mirrors
...
// regenerate the browser fallback bundles, and a French B1 equivalent of
...
const { units, language, level, courseTitle, courseDescription } = require('./content/french-b1-units');
...
function buildActivityRow(unit, skill, orderInUnit) {
  ...
  return {
    slug: `french-b1-${unit.slug}-${skill}`,
...
function main() {
  ...
  console.log(
    `Replaced ${removedCount} legacy French B1 row(s) with ${newRows.length} unit-scoped activities (${units.length} units).`
  );
