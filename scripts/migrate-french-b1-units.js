#!/usr/bin/env node
// scripts/migrate-french-b1-units.js
// Pushes French B1's unit-based content (built by
// scripts/build-french-b1-seed.js into lib/seed-lessons.json +
...
// scripts/migrate-french-a1-units.js exactly, with LEVEL='B1' and a unit
// count assertion that tracks however many B1 units currently exist in
// scripts/content/french-b1-units.js (grows as more units are authored).
...
// Usage: node scripts/migrate-french-b1-units.js
...
async function main() {
  ...
  if (lessons.length !== units.length * 7) {
    console.error(
      `Esperaba ${units.length * 7} actividades de French B1 en el seed, encontré ${lessons.length}. Abortando.`
    );
    process.exit(1);
  }

  console.log(`Migrando ${units.length} unidades y ${lessons.length} actividades de French B1...`);
  ...
  const { data: level, error: levelError } = await supabase
    .from('levels')
    .upsert({ code: LEVEL, name: 'B1 - Intermédiaire', sort_order: 3 }, { onConflict: 'code' })
    .select('id')
    .single();
  ...
  console.log('Migración de French B1 (unidades) completa.');
}

main().catch((error) => {
  console.error('Error migrando French B1 (unidades):', error.message);
  process.exit(1);
});
