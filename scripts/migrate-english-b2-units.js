...
const lessons = seedLessons.filter((row) => row.target_language === 'english' && row.level === 'B2');
...
if (units.length !== 12 || lessons.length !== 72) throw new Error('Build English B2 seed first.');
  ...
  try {
    ...
    const level = (await client.query(
      `insert into levels(code,name,sort_order) values('B2','B2 - Upper intermediate',4)
    ...
    const course = (await client.query(
      ...
      [language.id, level.id, 'English B2', 'Upper-intermediate English through social issues, evidence and debate.']
    )).rows[0];
    ...
    for (const row of lessons) {
      ...
      const lessonId = (await client.query(
        ...
        [course.id, unitIds[row.unit_slug], row.slug, row.skill, row.title, row.description || '',
          row.order_index, content.xp_reward || 20, row.access_tier, row.estimated_minutes || 15,
    ...
    console.log('English B2 migration complete:', result);
  } catch (error) {
