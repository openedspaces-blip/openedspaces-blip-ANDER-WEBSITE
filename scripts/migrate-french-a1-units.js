...
// Pushes French A1's unit-based content (built by
// scripts/build-french-a1-seed.js into lib/seed-lessons.json +
...
// scripts/migrate-english-a1-units.js; kept as a separate sibling script
// (not a shared module) so English's working migration is never touched.
// Requires supabase/migrations/202607170001_course_units.sql (course_units)
// and supabase/migrations/202607200001_dialogue_skill_and_mission_fields.sql
// (mission/grammar_note/phrases columns + the 'dialogue' skill) to have
// been applied first.
...
// part of the new 84-activity set.
// Usage: node scripts/migrate-french-a1-units.js
...
async function main() {
  ...
  if (units.length !== 12) {
    console.error(`Esperaba 12 unidades de French A1 en el seed, encontré ${units.length}. Abortando.`);
    process.exit(1);
  }
  if (lessons.length !== units.length * 7) {
    console.error(
      `Esperaba ${units.length * 7} actividades de French A1 en el seed, encontré ${lessons.length}. Abortando.`
    );
    process.exit(1);
  }

  console.log(`Migrando ${units.length} unidades y ${lessons.length} actividades de French A1...`);
  ...
  const { data: level, error: levelError } = await supabase
    .from('levels')
    .upsert({ code: LEVEL, name: 'A1 - Débutant', sort_order: 1 }, { onConflict: 'code' })
    .select('id')
    .single();
    ...
    .upsert(
      {
        ...
        title: 'Français A1',
        description:
          "Français de survie : salutations, présentations, famille, vie quotidienne et premières sorties, organisés en 12 unités thématiques."
      },
  ...
  for (const row of lessons) {
    ...
    if (content.reading) {
      sectionRows.push({
        ...
        // columns - pass arrays/objects as-is, do not JSON.stringify() them
        // (that would double-encode the value).
  ...
  // new 84-activity set. Cascade-deletes their sections/exercises/options
  // and any user progress against them.
  ...
  console.log('Migración de French A1 (unidades) completa.');
}

main().catch((error) => {
  console.error('Error migrando French A1 (unidades):', error.message);
  process.exit(1);
});
