#!/usr/bin/env node
// scripts/migrate-english-a2-units.js
// Pushes English A2's unit-based content (built by
// scripts/build-english-a2-seed.js into lib/seed-lessons.json +
...
// scripts/migrate-english-a1-units.js / migrate-french-a1-units.js; kept as
// a separate sibling script (not a shared module) so English A1's working
// migration is never touched by A2-specific changes.
...
// part of the current seed (only relevant once units are replaced/renamed
// across phases).
// Usage: node scripts/migrate-english-a2-units.js
...
async function main() {
  ...
  if (!units.length) {
    console.error('No hay unidades de English A2 en el seed. Corre build-english-a2-seed.js primero.');
    process.exit(1);
  }
  // Phase 1 only ships Unit 1 - unlike the A1/French scripts, this does not
  // hardcode an expected total (12 units), just checks internal consistency
  // (every unit really has all 6 core skills), so later phases adding units
  // 2-10 never need this number updated.
  if (lessons.length !== units.length * 6) {
    console.error(
      `Esperaba ${units.length * 6} actividades de English A2 en el seed (${units.length} unidad(es) x 6 skills), encontré ${lessons.length}. Abortando.`
    );
    process.exit(1);
  }

  console.log(`Migrando ${units.length} unidad(es) y ${lessons.length} actividad(es) de English A2...`);
  ...
  const { data: level, error: levelError } = await supabase
    .from('levels')
    .upsert({ code: LEVEL, name: 'A2 - Elementary', sort_order: 2 }, { onConflict: 'code' })
    .select('id')
    .single();
    ...
    .upsert(
      {
        ...
        title: 'English A2',
        description:
          'Everyday communication in English: routines, family, home, food, past experiences, travel, health, plans, work and personal stories, organized in thematic units.'
      },
  ...
  for (const row of lessons) {
    ...
    if (content.reading) {
      sectionRows.push({
        ...
        // jsonb columns - pass arrays/objects as-is, see
        // migrate-english-a1-units.js's comment for why JSON.stringify()-ing
        // them here would double-encode and break the client's .map(...).
  ...
  // current seed (relevant once a later phase renames/replaces a unit).
  ...
  console.log('Migración de English A2 (unidades) completa.');
}

main().catch((error) => {
  console.error('Error migrando English A2 (unidades):', error.message);
  process.exit(1);
});
