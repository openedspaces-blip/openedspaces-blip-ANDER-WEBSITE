#!/usr/bin/env node
// scripts/migrate-french-a2-units.js
// Pushes French A2's unit-based content (built by
// scripts/build-french-a2-seed.js into lib/seed-lessons.json +
...
// scripts/migrate-french-a1-units.js exactly, with LEVEL='A2' and a unit
// count assertion that tracks however many A2 units currently exist in
// scripts/content/french-a2-units.js (grows as more units are authored).
...
// Usage: node scripts/migrate-french-a2-units.js
...
async function main() {
  ...
  if (lessons.length !== units.length * 7) {
    console.error(
      `Esperaba ${units.length * 7} actividades de French A2 en el seed, encontré ${lessons.length}. Abortando.`
    );
    process.exit(1);
  }

  console.log(`Migrando ${units.length} unidades y ${lessons.length} actividades de French A2...`);
  ...
  const { data: level, error: levelError } = await supabase
    .from('levels')
    .upsert({ code: LEVEL, name: 'A2 - Élémentaire', sort_order: 2 }, { onConflict: 'code' })
    .select('id')
    .single();
  ...
  console.log('Migración de French A2 (unidades) completa.');
}

main().catch((error) => {
  console.error('Error migrando French A2 (unidades):', error.message);
  process.exit(1);
});
