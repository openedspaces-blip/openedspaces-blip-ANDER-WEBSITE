#!/usr/bin/env node
// scripts/build-french-a2-seed.js
// Folds scripts/content/french-a2-units.js (hand-authored units) into
// lib/seed-lessons.json/lib/seed-units.json, additively - only rows with
// target_language 'french' and level 'A2' are replaced, so this never
// touches the existing French A1 rows. Mirrors
...
// regenerate the browser fallback bundles, and a French A2 equivalent of
...
const { units, language, level, courseTitle, courseDescription } = require('./content/french-a2-units');
...
function buildActivityRow(unit, skill, orderInUnit) {
  ...
  return {
    slug: `french-a2-${unit.slug}-${skill}`,
...
function main() {
  ...
  console.log(
    `Replaced ${removedCount} legacy French A2 row(s) with ${newRows.length} unit-scoped activities (${units.length} units).`
  );
