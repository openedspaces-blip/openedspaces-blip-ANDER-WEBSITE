...
function sanitizeGrammarTestQuestion(question) {
  ...
  const { id, type, prompt, difficulty, l1Hint } = question;
  const base = { id, type, prompt };
  if (difficulty) base.difficulty = difficulty;
  // L1 hints are authored learner support, never an answer key. They are
  // especially useful for German Listening, where a Spanish bridge helps
  // learners focus on meaning before they choose an option.
  if (l1Hint) base.l1Hint = l1Hint;
  if (type === 'mcq') {
