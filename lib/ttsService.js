...
// Premium learners; ElevenLabs is the server-side neural fallback.
const OpenAI = require('openai');

const DEFAULT_MODEL = process.env.OPENAI_TTS_MODEL || 'gpt-4o-mini-tts';
const DEFAULT_VOICE =
  process.env.OPENAI_TTS_VOICE || (DEFAULT_MODEL === 'gpt-4o-mini-tts' ? 'marin' : 'nova');
...
function isConfigured() {
  return isGeminiConfigured() || isElevenLabsConfigured() || isAzureConfigured() || isOpenAiConfigured();
}
...
function voiceForLanguage(language = 'english') {
  ...
  return isAzureConfigured() ? (AZURE_VOICES[language] || AZURE_VOICES.english) : DEFAULT_VOICE;
}
...
function readingInstructions(language) {
  const names = {
    english: 'English', spanish: 'Latin American Spanish', french: 'French',
    german: 'German', italian: 'Italian', portuguese: 'Brazilian Portuguese'
  };
  return `Read naturally and clearly in ${names[language] || 'the requested language'} for a language learner. Use a warm, measured teaching pace, natural phrasing, and precise pronunciation. Do not add any words.`;
}
...
async function generateReadingSpeech(text, { language = 'english' } = {}) {
  if (!isConfigured()) {
  ...
  for (const chunk of chunks) {
    ...
    // `instructions` is a gpt-4o-mini-tts feature. Keeping legacy models
    // valid here lets an existing OPENAI_TTS_MODEL continue as a fallback.
    if (DEFAULT_MODEL === 'gpt-4o-mini-tts') request.instructions = readingInstructions(language);
    const response = await getClient().audio.speech.create(request);
