...
async function createCheckoutTransaction({ userId, billingCycle }) {
  ...
  if (!webhookConfigured()) {
    const error = new Error('Paddle webhook verification is not configured.');
    error.status = 503;
    throw error;
  }
  const priceId = priceIdForBillingCycle(billingCycle);
  if (!priceId) {
    const error = new Error('Invalid or unavailable billing cycle.');
    error.status = 400;
    throw error;
  }
