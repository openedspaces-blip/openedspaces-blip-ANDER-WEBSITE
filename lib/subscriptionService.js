...
async function getSubscriptionSummary(userId) {
  ...
  const [subscription, tutorQuery, tutorVoice, paddleReference] = await Promise.all([
...
async function syncProviderSubscription({
  ...
  const normalizedStatus = ['active', 'trialing', 'past_due', 'canceled', 'expired'].includes(status)
    ? status
    : 'past_due';
  ...
  if (error) throw new Error(`No se pudo sincronizar la suscripciÃ³n Paddle: ${error.message}`);
...
async function syncPaddleSubscription({
  ...
  const normalizedPlan = plan === 'quarterly' ? 'quarterly' : 'monthly';
  const planId = await planService.getPlanIdBySlug('premium');
  if (!planId) throw new Error("Plan 'premium' no existe en public.plans.");
