...
async function syncPaddleSubscription({
  ...
  const write = existing
    ? admin.from('subscriptions').update(payload).eq('id', existing.id).select().single()
    : admin.from('subscriptions').insert(payload).select().single();
  let { data: subscription, error } = await write;

  // Paddle can fire multiple webhook events for the same checkout
  // (transaction.completed and subscription.created/activated) close
  // enough together that both requests see `existing` as null and both
  // attempt an insert. Postgres 23505 here means a sibling request already
  // won the insert - refetch by paddle_subscription_id (now guaranteed to
  // exist) and fall back to an update instead of surfacing a false failure.
  if (error?.code === '23505' && !existing) {
    const { data: nowExisting, error: refetchError } = await admin
      .from('subscriptions')
      .select('id')
      .eq('paddle_subscription_id', paddleSubscriptionId)
      .maybeSingle();
    if (!refetchError && nowExisting) {
