...
function isProfilesTableError(error) {
  return /profiles table is not ready|public\.profiles|schema cache|relation .*profiles/i.test(
    error?.message || ''
  );
}

function isLessonsTableError(error) {
  return /public\.lessons|public\.lesson_completions|schema cache|relation .*lessons|relation .*lesson_completions/i.test(
    error?.message || ''
  );
}
...
function publicLesson(lesson, completedSlugs = new Set()) {
  ...
  return {
    ...
    accessTier:
      lesson.access_tier ||
      lesson.accessTier ||
      ((lesson.is_free ?? lesson.isFree ?? true) ? 'free' : 'premium'),
...
async function readLessons(admin, filters = {}) {
  let query = admin
    .from('lessons')
    .select(
      'id, slug, target_language, level, skill, title, description, order_index, is_free, is_published, estimated_minutes, xp_reward, access_tier, payment_price_usd, content_json'
    )
    .order('order_index', { ascending: true });

  if (filters.language || filters.target_language)
    query = query.eq('target_language', filters.language || filters.target_language);
...
function readSeedLessonsWithFilters(filters = {}) {
  return readSeedLessons()
    .filter(
      (lesson) =>
        (!filters.language && !filters.target_language) ||
        lesson.target_language === (filters.language || filters.target_language)
    )
...
function defaultProfileForUser(user, name) {
  return {
    full_name:
      name || user?.user_metadata?.name || user?.email?.split('@')[0] || user?.email || 'Usuario',
...
function sendProfilesSetupError(res, error) {
  return sendJson(res, 503, {
    error: error.message,
    manualAction:
      'Run SUPABASE_RUN_THIS.sql in the Supabase SQL editor, or set SUPABASE_DATABASE_URL and run npm run db:setup.'
  });
}
...
async function handleHealth(req, res) {
  ...
  try {
    ...
    return sendJson(res, 200, {
      ...
      configured: true,
      warning,
      manualAction: profilesTableChecked
        ? undefined
        : 'Run SUPABASE_RUN_THIS.sql in the Supabase SQL editor, or set SUPABASE_DATABASE_URL and run npm run db:setup.',
      services: {
        supabaseAuth: true,
...
async function handleLogout(req, res) {
  ...
  try {
    ...
    if (token) {
      await supabaseAdmin()
        .auth.admin.signOut(token, 'global')
        .catch(() => {});
    }
...
async function handleUser(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Social providers create the Auth user before ANDERGO has a chance to
      // create its application profile. Complete that provisioning here so
      // the first Google sign-in has the same defaults as email/password.
      if (!profile) profile = await upsertProfile(admin, auth.user);
    } catch (profileError) {
      if (!isProfilesTableError(profileError)) {
        throw profileError;
      }
...
async function handleCompleteLesson(req, res) {
  ...
  try {
    ...
    const admin = supabaseAdmin();
    let lessons = await readLessons(admin, {
      target_language: req.query?.language || 'english',
      level: 'A1'
    });
    const lesson =
      lessons.find((item) => item.slug === slug) || (await readLessons(admin, { slug }))[0];

    if (!lesson) return sendJson(res, 404, { error: 'Lesson not found.' });

    const { error: completionError } = await admin.from('lesson_completions').upsert(
      {
    ...
    if (isLessonsTableError(error)) {
      return sendJson(res, 503, {
        error: error.message,
        manualAction:
          'Run supabase/migrations/202601010001_andergo_lessons.sql in the Supabase SQL editor.'
      });
    }
