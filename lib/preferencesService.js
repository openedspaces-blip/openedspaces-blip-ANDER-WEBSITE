...
// learner already speaks, used for explanations/hints - it must always
// differ from the target language, enforced both here and by a DB CHECK
...
async function getPreferences(userId) {
  if (config.isSupabaseConfigured) {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('preferred_language, preferred_level, bridge_language')
      .eq('id', userId)
      .maybeSingle();
    return {
...
async function updatePreferences(userId, { language, level, bridgeLanguage }) {
  ...
  // Bridge and target can never match - resolve against whichever value
  // isn't being changed in this call, since the client may only send one
  // of the two fields.
  ...
  if (nextLanguage === nextBridge) {
    throw httpError('El idioma puente debe ser diferente del idioma que deseas aprender.', 400);
  }
