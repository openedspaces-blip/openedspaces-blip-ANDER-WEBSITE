...
async function createResponse({ input, instructions, model = DEFAULT_MODEL, maxOutputTokens, signal }) {
  ...
  // Responses marked incomplete can contain a short prefix such as
  // "porque es". Treat it as a failed attempt so aiTutorService can use a
  // healthy fallback instead of presenting a cut-off answer to the learner.
  if (response.status === 'incomplete') {
    const err = new Error('La respuesta del Tutor quedó incompleta.');
    err.status = 502;
    throw err;
  }
  if (!text) {
