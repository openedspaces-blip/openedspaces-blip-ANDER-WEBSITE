...
function getEnvVars() {
  return {
    ...
    supabaseDatabaseUrl: config.supabaseDatabaseUrl
  };
}
...
function getSupabaseConfigError() {
  ...
  if (!supabaseUrl || isPlaceholderValue(supabaseUrl)) {
    return 'Missing or invalid SUPABASE_URL in environment variables. You can also use NEXT_PUBLIC_SUPABASE_URL. Expected format: https://your-project-id.supabase.co';
  }
  if (!supabaseAnonKey || isPlaceholderValue(supabaseAnonKey)) {
    return 'Missing or invalid SUPABASE_ANON_KEY in environment variables. You can also use SUPABASE_KEY or NEXT_PUBLIC_SUPABASE_ANON_KEY. Expected format: eyJ... (Publishable Key)';
  }
