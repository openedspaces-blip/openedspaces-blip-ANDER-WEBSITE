...
async function generateListeningPractice({ bridgeLanguage, targetLanguage, level, topic, durationSeconds }) {
  ...
  if (!isListeningGenerationConfigured()) {
    const err = new Error('La practica de Listening con IA no esta configurada todavia (falta GEMINI_API_KEY u OPENAI_API_KEY).');
    err.status = 503;
    throw err;
  }

  const boundedDuration = Math.max(10, Math.min(Number(durationSeconds) || 30, MAX_DURATION_SECONDS));
  ...
  const { text } = await createResponse({
    instructions: 'Eres un generador de guiones cortos de listening para estudiantes de idiomas. Respondes unicamente con JSON valido, sin explicaciones adicionales.',
  ...
  try {
    const slowBuffer = await generateSpeechMp3(parsed.script, { language: targetLanguage, speed: 0.75 });
