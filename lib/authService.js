...
const SIGNUP_ERROR_MESSAGES = {
  ...
  over_email_send_rate_limit:
    'Has solicitado demasiados correos. Espera un momento y vuelve a intentarlo.',
  signup_disabled: 'El registro no está disponible en este momento.'
};
const SIGNUP_SEND_FAILURE_MESSAGE = 'No pudimos enviar el correo en este momento. Intenta nuevamente.';
  ...
  if (config.isSupabaseConfigured) {
    ...
    if (error) {
      if (isDuplicateEmailError(error)) {
        console.warn(`[auth] signup blocked requestId=${requestId} code=email_exists`);
      ...
      console.warn(
        `[auth] signup failed requestId=${requestId} code=${error.code || error.status || 'unknown'}`
      );
    ...
    console.log(`[auth] signup accepted requestId=${requestId}`);
...
async function resendConfirmation(email) {
  ...
  try {
    ...
    console.log(`[auth] resend accepted requestId=${requestId}`);
  } catch (error) {
    // code distinguishes "Supabase's own rate limit fired" (e.g.
    // over_email_send_rate_limit, matching the dashboard's configured
    // minimum interval) from a real SMTP/provider failure - never the
    // email itself, and the response to the caller stays neutral either way.
    console.warn(
      `[auth] resend failed requestId=${requestId} code=${error?.code || error?.status || 'unknown'}`
    );
  }
...
async function verifyOtp({ email, token, purpose = 'signup' }) {
  ...
  if (error) {
    // code distinguishes "wrong code" from "expired code" (otp_expired) in
    // the logs without ever logging the code itself - see OTP_ERROR_MESSAGES
    // above for the user-facing text these map to.
    console.warn(`[auth] otp verify failed requestId=${requestId} code=${error.code || 'unknown'}`);
  ...
  console.log(`[auth] otp verified requestId=${requestId}`);
...
async function requestPasswordReset(email) {
  ...
  try {
    ...
    console.log(
      `[auth] password reset request: requestId=${requestId} status=ok errorCode=none durationMs=${Date.now() - startedAt}`
    );
  } catch (error) {
    console.log(
      `[auth] password reset request: requestId=${requestId} status=error errorCode=${error?.code || 'unknown'} durationMs=${Date.now() - startedAt}`
    );
