...
// Every requireAuth-gated request used to call Supabase's auth.getUser(token),
// a network round-trip to GoTrue on every single API call - measured at
// 1.5-5.5s per request in production, the main cause behind login "feeling
// stuck" after the credentials themselves already checked out. Supabase
// access tokens are JWTs signed with the project's own key (ES256 here);
// verifying the signature locally against the project's public JWKS - fetched
// once and cached in-process by jose - avoids that round-trip on every
// request while providing the exact same guarantee (only Supabase could have
// produced a validly-signed token for this project).
let jwks = null;
function getJwks() {
...
// Returns the same public identity shape verifyToken already returns
// from auth.getUser(), or null if the token doesn't verify - never throws.
async function verifyTokenLocally(token) {
  try {
    ...
    return {
      ...
      name: payload.user_metadata?.name || payload.user_metadata?.full_name || null,
      username: payload.user_metadata?.username || null
    };
  } catch {
    // Covers expiry, bad signature, unknown/rotated kid the JWKS fetch
    // itself failed to resolve, clock skew beyond jose's tolerance, etc. -
    // all of these mean "not a currently-valid session", the same outcome
    // verifyToken already returns for an invalid token.
    return null;
  }
}
...
const SIGNUP_ERROR_MESSAGES = {
  weak_password: 'Esta contraseña no fue aceptada. Prueba una diferente, única y más larga, con letras, números y un símbolo.',
...
function validateRegistrationPassword(password) {
  ...
  if (value.length < 8 || !/[A-Za-z]/.test(value) || !/\d/.test(value)) {
    throw httpError(
      'Usa al menos 8 caracteres e incluye al menos una letra y un número.',
      400
    );
  }
}
...
async function register({ email, password, name, username }) {
  ...
  if (config.isSupabaseConfigured) {
    ...
    if (data.session) {
      await welcomeEmailService.sendAfterRegistration({
        userId: data.user.id,
