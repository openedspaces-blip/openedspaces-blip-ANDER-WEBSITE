...
const vocabularyDictionaryService = require('./vocabularyDictionaryService');
...
function createServer() {
  ...
  // General dictionary: independent from course routes and read-only for the
  // browser. Entries are curated/imported server-side in Spanish translation.
  app.get('/api/dictionary', async (req, res) => {
    try {
      const result = await vocabularyDictionaryService.listEntries({
      ...
      res.status(error.status || 500).json({ ok: false, error: error.message || 'No se pudo consultar el diccionario.' });
    }
  });
