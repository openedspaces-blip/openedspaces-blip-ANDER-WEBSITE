...
function buildVocabularyQuestion(item, wordRows, index, language) {
  ...
  const prompt =
    language === 'french'
      ? `Comment dit-on « ${definition} » en français ?`
      : language === 'spanish'
        ? `¿Cómo se dice « ${definition} » en español?`
        : language === 'italian'
          ? `Come si dice « ${definition} » in italiano?`
          : language === 'portuguese'
            ? `Como se diz “${definition}” em português?`
            : language === 'german'
              ? `Wie sagt man „${definition}“ auf Deutsch?`
              : `How do you say “${definition}” in English?`;
...
function buildTestLexicalQuestion(item, pool, index, language, area, idPrefix) {
  const question = buildVocabularyQuestion(item, pool, index, language);
  return { ...question, id: `${idPrefix}-${index + 1}`, area };
}
...
function buildLanguageLessonTest(language, level, unitSlug) {
  ...
  const vocabularyQuestions = Array.from(
    ...
    .filter(Boolean)
    .map((item, index) =>
      buildTestLexicalQuestion(item, lexicalPool, index, language, 'Vocabulary', 'v')
    );
  const adjectiveCount = level === 'A1' ? 3 : 2;
  const adjectiveQuestions = adjectivePool.slice(0, adjectiveCount).map((item, index) =>
    buildTestLexicalQuestion(item, lexicalPool, index + 3, language, 'Adjectives', 'adj')
  );
  const adverbQuestions = adverbPool.slice(0, 2).map((item, index) =>
    buildTestLexicalQuestion(item, lexicalPool, index + 5, language, 'Adverbs', 'adv')
  );
