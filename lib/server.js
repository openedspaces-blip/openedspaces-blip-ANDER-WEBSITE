...
function buildLanguageLessonTest(language, level, unitSlug) {
  const englishAssessment = language === 'english';
  const grammarQuestionCount = englishAssessment ? 7 : 9;
  const vocabularyQuestionCount = 3;
  const verbQuestionCount = englishAssessment && level !== 'A1' ? 2 : 3;
  ...
  let vocabularyQuestions = wordRows
    .slice(0, vocabularyCount)
    .map((item, index) =>
      language === 'italian' &&
      /^(palabra cultural|acción cotidiana|lugar o cosa|expresión útil)$/i.test(
  ...
  let adjectiveQuestions = [];
  let adverbQuestions = [];
  if (englishAssessment) {
    const levelLexicalRows = seedLessons
      .filter(
        (row) =>
          row.target_language === 'english' && row.level === level && row.skill === 'vocabulary'
      )
      .flatMap((row) => row.content_json?.vocabulary || [])
      .map((item) => ({
        word: item.targetWord || item.word,
      ...
      .filter((item) => item.word && item.translation);
    const curatedAdjectives = (ENGLISH_TEST_ADJECTIVES[level] || ENGLISH_TEST_ADJECTIVES.A1).map(
      ([word, definition]) => ({ word, translation: definition, definition, partOfSpeech: 'adjective' })
    );
    const lexicalPool = [...wordRows, ...levelLexicalRows, ...curatedAdjectives].filter(
      (item, index, list) =>
        list.findIndex(
          (candidate) =>
            String(candidate.word).toLocaleLowerCase() === String(item.word).toLocaleLowerCase()
        ) === index
    );
    const select = (kind, count) => {
      const candidates = lexicalPool.filter((item) => englishPartOfSpeech(item) === kind);
      return Array.from({ length: count }, (_unused, index) => candidates[index % candidates.length]).filter(Boolean);
    };
    const vocabularyItems = select('vocabulary', 3);
    const adjectiveCount = level === 'A1' ? 3 : 2;
    const adjectiveCandidates = lexicalPool.filter(
      (item) => englishPartOfSpeech(item) === 'adjective'
    );
    const adjectiveItems = Array.from(
      { length: adjectiveCount },
      (_unused, index) => adjectiveCandidates[index % adjectiveCandidates.length]
    ).filter(Boolean);
    vocabularyQuestions = vocabularyItems.map((item, index) =>
      buildEnglishLexicalQuestion(item, lexicalPool, index, 'Vocabulary', 'v')
    );
    adjectiveQuestions = adjectiveItems.map((item, index) =>
      buildEnglishLexicalQuestion(item, lexicalPool, index + 3, 'Adjectives', 'adj')
    );
    if (level !== 'A1') {
      const adverbs = ENGLISH_TEST_ADVERBS[level] || ENGLISH_TEST_ADVERBS.A2;
      adverbQuestions = adverbs.slice(0, 2).map(([word, definition], index) =>
        buildEnglishLexicalQuestion(
          { word, translation: definition, definition, partOfSpeech: 'adverb' },
          adverbs.map(([entry, meaning]) => ({ word: entry, translation: meaning, definition: meaning })),
          index + 5,
          'Adverbs',
          'adv'
        )
...
function createServer() {
  ...
  app.post('/api/interpreter/session', voiceInterpreterLimiter, async (req, res) => {
  ...
  app.post('/api/billing/checkout', requireAuth, async (req, res) => {
    try {
      ...
      const providerMessages = {
        ...
        forbidden: 'La API key de Paddle no tiene permiso Transactions: Write.',
        unauthorized: 'La API key Live de Paddle no es válida para esta cuenta.'
      };
      res.status(status).json({
  ...
  app.post('/api/billing/portal', requireAuth, async (req, res) => {
    try {
      ...
      res.status(status).json({
        error:
          status >= 500 ? 'No se pudo abrir la administración segura de tu plan.' : error.message
      });
    }
  });
  ...
  app.post('/api/billing/pause', requireAuth, async (req, res) => {
    try {
      ...
      res.status(status).json({
        error: status >= 500 ? 'No se pudo programar la pausa de tu plan.' : error.message
      });
    }
  });
