...
function buildLanguageLessonTest(language, level, unitSlug) {
  ...
  const grammarName = grammar?.content_json?.grammarProfile?.name || grammar?.content_json?.title || 'Grammar';
  ...
  const grammarQuestions = rawGrammarItems
    ...
    .map((item, index) => ({ id: `g-${index}`, area: 'Grammar', prompt: makeStandaloneGrammarPrompt(item.prompt, grammarName, language), options: item.options, answer: item.answer }));
  ...
  const verbPool = language === 'french'
    ? frenchTestVerbs
    : language === 'spanish'
      ? spanishTestVerbs
      : englishTestVerbs;
  ...
  const verbQuestions = selectedVerbs.map((verb, index) => {
    if (index % 2 === 0) {
      ...
      const tenseLabel = language === 'french'
        ...
        : language === 'spanish'
          ? (formKey === 'pastSimple' ? 'pretérito' : 'participio')
        : (formKey === 'pastSimple' ? 'Past Simple' : 'Past Participle');
      const infinitive = verb.infinitive || verb.baseForm;
      const prompt = language === 'french'
        ? `Choisissez la forme au ${tenseLabel} du verbe « ${infinitive} ».`
        : language === 'spanish'
          ? `Elige la forma en ${tenseLabel} del verbo « ${infinitive} ».`
          : `Choose the ${tenseLabel} form of “${infinitive}”.`;
      return { id: `verb-${index}`, area: 'Verbs', prompt, options: ordered, answer: ordered.indexOf(correct) };
    }
    const translationKey = language === 'spanish' ? 'english' : 'spanish';
    const correct = verb.translation?.[translationKey] || '';
    const distractors = eligibleVerbs.filter((item) => item.id !== verb.id).slice((unitSeed + 7) % 25, ((unitSeed + 7) % 25) + 3).map((item) => item.translation?.[translationKey]).filter(Boolean);
    const options = [correct, ...distractors];
    const rotation = (unitSeed + 1) % options.length;
    const ordered = [...options.slice(rotation), ...options.slice(0, rotation)];
    const infinitive = verb.infinitive || verb.baseForm;
    const prompt = language === 'french'
      ? `Que signifie le verbe « ${infinitive} » ?`
      : language === 'spanish'
        ? `¿Qué significa el verbo « ${infinitive} »?`
        : `What does the verb “${infinitive}” mean?`;
    return { id: `verb-${index}`, area: 'Verbs', prompt, options: ordered, answer: ordered.indexOf(correct) };
  });
  // The lesson exam evaluates the language system taught in the unit. Reading
  // comprehension owns its separate question bank and must not leak into this
  // assessment. Keep the three assessed areas explicit and balanced here.
...
function createServer() {
  ...
  // TEMPORARY diagnostic route - remove once the AI Tutor provider
  // configuration is confirmed correct on Render. Never returns full secrets.
  app.get('/api/_debug/ai-key-shape', (_req, res) => {
    const shape = (name) => {
    ...
    res.json({
      CEREBRAS_API_KEY: shape('CEREBRAS_API_KEY'),
      GROQ_API_KEY: shape('GROQ_API_KEY'),
  ...
  app.get('/api/tests', (req, res) => {
    const language = String(req.query.language || 'english').toLowerCase();
    const level = String(req.query.level || 'A1').toUpperCase();
    if (!['english', 'french', 'spanish'].includes(language) || !['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].includes(level)) {
