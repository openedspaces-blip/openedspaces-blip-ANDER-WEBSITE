...
function createServer() {
  ...
  app.post('/api/ai/tutor', async (req, res) => {
    try {
      const {
        ...
        transcript, vocabulary, currentQuestion, selectedAnswer, supportMode
      } = req.body || {};
      ...
      if (error.code === 'AI_PROVIDER_TEMPORARILY_UNAVAILABLE') {
        return res.status(503).json({ ok: false, error: error.code, message: error.message });
      }
  ...
  app.post('/api/lessons/:slug/check-answer', attachUserIfPresent, async (req, res) => {
    try {
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ success: false, error: error.message || 'No se pudo verificar la respuesta.' });
    }
  });
  ...
  const listeningGenerateLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    message: { success: false, error: 'Se alcanzó el límite de generación de práctica de Listening. Intenta de nuevo en unos minutos.' }
  });
  ...
  app.get('/api/listening/audio', attachUserIfPresent, async (req, res) => {
    try {
      ...
      res.status(500).json({ status: 'unavailable', error: 'No se pudo consultar el audio de Listening.' });
    }
  });
  ...
  app.post('/api/listening/generate', requireAuth, listeningGenerateLimiter, async (req, res) => {
    try {
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ success: false, error: error.message || 'No se pudo generar la práctica de Listening.' });
    }
  });
