...
function createServer() {
  ...
  // TEMPORARY - confirm GROQ_API_KEY landed on Render, same pattern as the
  // earlier CEREBRAS_API_KEY check. Never returns the full secret.
  app.get('/api/_debug/groq-key-shape', (_req, res) => {
    const raw = process.env.GROQ_API_KEY || '';
    res.json({ set: raw.length > 0, length: raw.length, startsWith: raw.slice(0, 5) });
  });
