...
function buildLanguageLessonTest(language, level, unitSlug) {
  ...
  const vocabularyCount = Math.min(
    wordRows.length,
    upperLevelExam ? sectionSize : Math.max(5, 10 - grammarQuestions.length)
  );
  ...
  const verbCount = upperLevelExam
    ? sectionSize
    : Math.max(2, 12 - grammarQuestions.length - vocabularyQuestions.length);
  ...
  return [...grammarQuestions, ...vocabularyQuestions, ...verbQuestions].slice(
    0,
    upperLevelExam ? sectionSize * 3 : 12
  );
}
