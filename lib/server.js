...
function createServer() {
  ...
  app.post('/api/client-events', (req, res) => {
    const allowedTypes = new Set(['error', 'unhandled-rejection', 'performance']);
  ...
  app.get('/api/verbs/progress', requireAuth, async (req, res) => {
    try {
      const progress = await verbProgressService.getDetailedProgress({
        userId: req.user.id,
        languageCode: String(req.query.language || 'english').toLowerCase()
      });
      res.json({ progress });
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ error: error.message || 'No se pudo cargar el progreso de Verbos.' });
    }
  });

  app.patch('/api/verbs/progress/:verbId', requireAuth, async (req, res) => {
    try {
      const progress = await verbProgressService.saveVerbState({
        userId: req.user.id,
        languageCode: String(req.body?.language || 'english').toLowerCase(),
        verbId: String(req.params.verbId || ''),
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ error: error.message || 'No se pudo guardar el verbo.' });
    }
  });

  app.post('/api/verbs/progress/:verbId/attempt', requireAuth, async (req, res) => {
    try {
      const progress = await verbProgressService.recordVerbAttempt({
        userId: req.user.id,
        languageCode: String(req.body?.language || 'english').toLowerCase(),
        verbId: String(req.params.verbId || ''),
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ error: error.message || 'No se pudo guardar el intento.' });
    }
  });

  app.delete('/api/verbs/progress', requireAuth, async (req, res) => {
    try {
      const result = await verbProgressService.resetDetailedProgress({
        userId: req.user.id,
        languageCode: String(req.query.language || 'english').toLowerCase()
      });
      res.json(result);
      ...
      const status = error.status && error.status >= 400 ? error.status : 500;
      res.status(status).json({ error: error.message || 'No se pudo reiniciar el progreso de Verbos.' });
    }
  });
  ...
  app.post('/api/billing/checkout', requireAuth, async (req, res) => {
    try {
      ...
      const status = error.status || 500;
      if (status >= 500) console.error('[paddle-checkout]', error.message);
      res.status(status).json({
        error:
          status >= 500
