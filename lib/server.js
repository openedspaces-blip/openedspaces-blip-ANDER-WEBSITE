...
function buildLanguageLessonTest(language, level, unitSlug) {
  ...
  const verbQuestions = selectedVerbs.map((verb, index) => {
    if (index % 2 === 0) {
      ...
      const tenseLabel = language === 'italian'
        ? (formKey === 'pastSimple' ? 'passato' : 'participio passato')
        : language === 'portuguese'
          ? (formKey === 'pastSimple' ? 'pretérito' : 'particípio passado')
          : language === 'german'
            ? (formKey === 'pastSimple' ? 'Präteritum' : 'Partizip II')
        : language === 'french'
      ...
      const prompt = language === 'italian'
        ? `Scegli la forma al ${tenseLabel} del verbo « ${infinitive} ».`
        : language === 'portuguese'
          ? `Escolha a forma no ${tenseLabel} do verbo « ${infinitive} ».`
          : language === 'german'
            ? `Wähle die Form im ${tenseLabel} des Verbs „${infinitive}“.`
        : language === 'french'
    ...
    const infinitive = verb.infinitive || verb.baseForm;
    const prompt = language === 'italian'
      ? `Che cosa significa il verbo « ${infinitive} »?`
      : language === 'portuguese'
        ? `O que significa o verbo « ${infinitive} »?`
        : language === 'german'
          ? `Was bedeutet das Verb „${infinitive}“?`
      : language === 'french'
