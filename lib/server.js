...
const { getOrCreateReadingAudio } = require('./readingAudioStorage');
const {
...
function createServer() {
  ...
  // Public, read-only Wikipedia bridge for Reading's “Explora más” panel.
  // Keeping a short server cache means students do not repeatedly hit
  // Wikimedia from every device and lets us send Wikimedia an identifiable
  // User-Agent, as requested by its API etiquette.
  const wikipediaSearchCache = new Map();
  const wikipediaLanguageCodes = new Set(['en', 'es', 'fr', 'de', 'it', 'pt', 'ht']);
  app.get('/api/wikipedia/search', async (req, res) => {
    const language = String(req.query.language || '').trim().toLowerCase();
    const query = String(req.query.q || '').replace(/\s+/g, ' ').trim().slice(0, 160);
    if (!wikipediaLanguageCodes.has(language) || !query) {
      return res.status(400).json({ error: 'Idioma o consulta inválidos.' });
    }
    ...
    try {
      ...
      endpoint.searchParams.set('srlimit', '4');
      endpoint.searchParams.set('srnamespace', '0');
      const response = await fetch(endpoint, {
        headers: {
          'User-Agent': 'ANDERGO Language Academy/1.0 (https://andergo.online; support@andergo.online)',
          'Api-User-Agent': 'ANDERGO Language Academy/1.0'
        }
      });
      if (!response.ok) throw new Error(`Wikipedia respondió ${response.status}.`);
      const payload = await response.json();
      const pages = (payload?.query?.search || []).map((page) => ({
        title: String(page.title || ''),
        description: String(page.snippet || '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim()
      })).filter((page) => page.title);
      wikipediaSearchCache.set(cacheKey, { createdAt: Date.now(), pages });
      res.set('Cache-Control', 'public, max-age=300, s-maxage=600').json({ pages });
    } catch (error) {
      console.warn('[wikipedia-search]', error.message);
      res.status(502).json({ error: 'No se pudo consultar Wikipedia ahora.' });
    }
  });
  ...
  // Create natural Reading audio only when an authenticated learner presses
  // play. Browser speech remains the fallback for any unavailable case.
  app.post(
    '/api/tts/reading',
    requireAuth,
    requirePremiumNaturalReading,
    rateLimit({ windowMs: 5 * 60 * 1000, max: 12, standardHeaders: true, legacyHeaders: false }),
    async (req, res) => {
      try {
        if (!isTtsConfigured())
          return res.status(503).json({ error: 'La voz natural no está disponible.' });
        const { text, language } = req.body || {};
        const normalized = String(text || '').replace(/\s+/g, ' ').trim();
        if (!normalized || normalized.length > MAX_READING_CHARS) {
          return res.status(400).json({ error: 'El texto excede el límite de voz natural.' });
        }
        const storedAudio = await getOrCreateReadingAudio({
          text: normalized,
          language,
          makeAudio: () => generateReadingSpeech(normalized, { language })
        });
        if (storedAudio?.signedUrl) {
          return res.json({ audioUrl: storedAudio.signedUrl, cached: storedAudio.cached });
        }
        // Local development without Supabase remains usable, but production
        // always stores and signs the narration through the private bucket.
        const audio = await generateReadingSpeech(normalized, { language });
        res.set({
          'Content-Type': audio.contentType || (ttsProvider() === 'gemini' ? 'audio/wav' : 'audio/mpeg'),
          'Content-Length': String(audio.length),
          'Cache-Control': 'private, max-age=3600'
        });
        return res.send(audio);
      } catch (error) {
        return res.status(error.status || 502).json({ error: 'No se pudo preparar la voz natural.' });
      }
    }
  );
  ...
  // Tests are a registered-member benefit through B1. Advanced assessments
  // (B2–C2) are Premium so their answer key, checking and grading endpoints
  // must share the exact same server-side rule as the catalogue UI.
  const requireTestAccess = async (req, res, next) => {
    const rawLevel = req.method === 'GET' ? req.query?.level : req.body?.level;
    const level = String(rawLevel || 'A1').toUpperCase();
    if (!['B2', 'C1', 'C2'].includes(level)) return next();
    try {
      const entitlements = await getUserEntitlements(req.user.id);
      if (entitlements.hasFullAccess) return next();
      return res.status(403).json({
        code: 'PREMIUM_REQUIRED',
        error: 'Las pruebas B2, C1 y C2 están disponibles con ANDERGO Premium.'
      });
    } catch {
      return res.status(503).json({ error: 'No se pudo verificar tu acceso Premium.' });
    }
  };
