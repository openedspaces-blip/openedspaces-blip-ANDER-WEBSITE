...
const { getOfficialReadingAudio, getOrCreateReadingAudio } = require('./readingAudioStorage');
const {
...
function createServer() {
  ...
  // Not every published Reading has its final studio narration uploaded yet.
  // In that case create one secure neural rendition once, cache it privately,
  // and return a signed URL. Later requests use that cached file, so playback
  // does not keep falling back to browser speech or repeatedly spend TTS.
  app.post(
    '/api/tts/reading',
    requireAuth,
    ...
    async (req, res) => {
      try {
        if (!isTtsConfigured()) {
          return res.status(503).json({ error: 'La voz natural no está disponible ahora.' });
        }
