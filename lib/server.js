...
function buildLanguageLessonTest(language, level, unitSlug) {
  ...
  const grammarQuestions = (grammar?.content_json?.exercises || [])
    .filter((item) => item.type === 'mcq' && Array.isArray(item.options) && Number.isInteger(item.answer))
    .slice(0, 5)
    .map((item, index) => ({ id: `g-${index}`, area: 'Grammar', prompt: item.prompt, options: item.options, answer: item.answer }));
  const wordRows = (vocabulary?.content_json?.vocabulary || [])
    .map((item) => ({ word: item.targetWord || item.word, translation: item.translation || item.meaning }))
    .filter((item) => item.word && item.translation);
  const vocabularyCount = Math.min(wordRows.length, Math.max(5, 10 - grammarQuestions.length));
  const vocabularyQuestions = wordRows.slice(0, vocabularyCount).map((item, index) => {
    const distractors = wordRows.filter((word) => word.word !== item.word).map((word) => word.translation);
    const options = [item.translation, ...distractors.slice(index, index + 3)];
    const rotation = index % options.length;
    const ordered = [...options.slice(rotation), ...options.slice(0, rotation)].slice(0, 4);
    return { id: `v-${index}`, area: 'Vocabulary', prompt: language === 'french' ? `Que signifie « ${item.word} » ?` : `What does “${item.word}” mean?`, options: ordered, answer: ordered.indexOf(item.translation) };
  });
  const unitSeed = [...unitSlug].reduce((sum, character) => sum + character.charCodeAt(0), 0);
  const verbPool = language === 'french' ? frenchTestVerbs : englishTestVerbs;
  const eligibleVerbs = verbPool.filter((verb) => level === 'A1' ? verb.level === 'A1' : true);
  const verbCount = Math.max(2, 12 - grammarQuestions.length - vocabularyQuestions.length);
  ...
  const verbQuestions = selectedVerbs.map((verb, index) => {
    if (index % 2 === 0) {
      const correct = verb.forms[formKey];
      const distractors = eligibleVerbs.filter((item) => item.id !== verb.id).slice((unitSeed + 3) % 20, ((unitSeed + 3) % 20) + 3).map((item) => item.forms[formKey]);
      const options = [correct, ...distractors];
      const rotation = unitSeed % options.length;
      const ordered = [...options.slice(rotation), ...options.slice(0, rotation)];
      const tenseLabel = language === 'french'
        ? (formKey === 'pastSimple' ? 'passé composé' : 'participe passé')
        : (formKey === 'pastSimple' ? 'Past Simple' : 'Past Participle');
      return { id: `verb-${index}`, area: 'Verbs', prompt: language === 'french' ? `Choisissez la forme au ${tenseLabel} du verbe « ${verb.infinitive} ».` : `Choose the ${tenseLabel} form of “${verb.baseForm}”.`, options: ordered, answer: ordered.indexOf(correct) };
    }
    const correct = verb.translation?.spanish || '';
    const distractors = eligibleVerbs.filter((item) => item.id !== verb.id).slice((unitSeed + 7) % 25, ((unitSeed + 7) % 25) + 3).map((item) => item.translation?.spanish).filter(Boolean);
    const options = [correct, ...distractors];
    const rotation = (unitSeed + 1) % options.length;
    const ordered = [...options.slice(rotation), ...options.slice(0, rotation)];
    return { id: `verb-${index}`, area: 'Verbs', prompt: language === 'french' ? `Que signifie le verbe « ${verb.infinitive} » ?` : `What does the verb “${verb.baseForm}” mean?`, options: ordered, answer: ordered.indexOf(correct) };
  });
  return [...grammarQuestions, ...vocabularyQuestions, ...verbQuestions].slice(0, 12);
}
...
async function getTutorPlanUsage(userId) {
...
function createServer() {
  ...
  app.get('/api/ai/tutor/usage', requireAuth, async (req, res) => {
    const usage = await getTutorPlanUsage(req.user.id);
  ...
  app.post('/api/ai/tutor', requireAuth, async (req, res) => {
    ...
    // Every successful learner-to-AI interaction consumes one query,
    // including the Speaking corrector.
    const tutorUsage = await getTutorPlanUsage(req.user.id);
    ...
    try {
      if (task === 'speaking_correction') {
        ...
        await getTutorReplyStream({
          ...
          topicLimit: Math.min(10, Math.max(1, Number(topicLimit) || 10)),
          transcript,
          vocabulary,
          currentQuestion,
          selectedAnswer,
  ...
  app.get('/api/tests', (req, res) => {
    const language = String(req.query.language || 'english').toLowerCase();
    const level = String(req.query.level || 'A1').toUpperCase();
    if (!['english', 'french'].includes(language) || !['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].includes(level)) {
      res.status(400).json({ error: language === 'spanish' ? 'Los tests de español estarán disponibles próximamente.' : 'Idioma o nivel de examen no válido.' });
      return;
    }
  ...
  app.get('/api/billing/config', (_req, res) => {
    res.json(billingService.getPublicConfig());
  });
