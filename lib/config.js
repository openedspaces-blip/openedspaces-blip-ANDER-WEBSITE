...
const requestedPaddleEnvironment = String(process.env.NEXT_PUBLIC_PADDLE_ENV || '')
  .trim()
  .toLowerCase();
...
module.exports = {
  ...
  premiumMonthlyPriceUsd:
    Number(process.env.PREMIUM_MONTHLY_PRICE_USD || process.env.PREMIUM_PRICE_USD) || 4.99,
  ...
  defaultSubscriptionPlan: ['free', 'premium'].includes(requestedDefaultPlan)
    ...
    : 'free',
  paddle: {
    // Intentionally no fallback: using the wrong Paddle account is worse
    // than disabling checkout with an explicit configuration error.
    environment: ['sandbox', 'production'].includes(requestedPaddleEnvironment)
      ? requestedPaddleEnvironment
      : null,
    environmentConfigured: ['sandbox', 'production'].includes(requestedPaddleEnvironment),
    clientSideToken: process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN || '',
    apiKey: process.env.PADDLE_API_KEY || '',
    webhookSecret: process.env.PADDLE_WEBHOOK_SECRET || '',
    monthlyPriceId: process.env.NEXT_PUBLIC_PADDLE_MONTHLY_PRICE_ID || '',
    quarterlyPriceId: process.env.NEXT_PUBLIC_PADDLE_QUARTERLY_PRICE_ID || '',
    tiers: {
      starter: {
        month: process.env.NEXT_PUBLIC_PADDLE_STARTER_MONTH_PRICE_ID || '',
        year: process.env.NEXT_PUBLIC_PADDLE_STARTER_YEAR_PRICE_ID || ''
      },
      pro: {
        month: process.env.NEXT_PUBLIC_PADDLE_PRO_MONTH_PRICE_ID || '',
        year: process.env.NEXT_PUBLIC_PADDLE_PRO_YEAR_PRICE_ID || ''
      },
      advanced: {
        month: process.env.NEXT_PUBLIC_PADDLE_ADVANCED_MONTH_PRICE_ID || '',
        year: process.env.NEXT_PUBLIC_PADDLE_ADVANCED_YEAR_PRICE_ID || ''
      }
    },
    checkoutUrl:
      process.env.PADDLE_CHECKOUT_URL ||
      (requestedPaddleEnvironment === 'production' ? 'https://andergo.online' : '')
  },
  // PayPal is an optional fallback while Paddle remains the primary
  // merchant-of-record checkout. Client ID and plan IDs are public checkout
  ...
  azul: {
    ...
    monthlyAmount: process.env.AZUL_MONTHLY_AMOUNT || '',
    quarterlyAmount: process.env.AZUL_QUARTERLY_AMOUNT || '',
