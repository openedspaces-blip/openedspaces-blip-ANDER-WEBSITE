...
module.exports = {
  ...
  aiTutorMonthlyLimitFree: positiveInteger(process.env.AI_TUTOR_MONTHLY_LIMIT_FREE, 15),
  ...
  translatorMonthlyLimitFree: positiveInteger(process.env.TRANSLATOR_MONTHLY_LIMIT_FREE, 15),
  translatorMonthlyLimitPremium: positiveInteger(process.env.TRANSLATOR_MONTHLY_LIMIT_PREMIUM, 500),
  ...
  // Learner-controlled practice reminders. The master switch stays off until
  // Resend and the Vercel cron secret have been configured in production.
  practiceReminders: {
    enabled: process.env.PRACTICE_REMINDERS_ENABLED === 'true',
    cronSecret: process.env.CRON_SECRET || '',
    apiKey: process.env.RESEND_API_KEY || '',
    from:
      process.env.PRACTICE_REMINDER_EMAIL_FROM ||
      process.env.WELCOME_EMAIL_FROM ||
      'ANDERGO Language Academy <no-reply@andergo.online>',
    replyTo:
      process.env.PRACTICE_REMINDER_EMAIL_REPLY_TO ||
      process.env.WELCOME_EMAIL_REPLY_TO ||
      'support@andergo.online',
    apiBaseUrl: String(process.env.RESEND_API_BASE_URL || 'https://api.resend.com').replace(/\/$/, ''),
    websiteUrl: String(process.env.PUBLIC_BASE_URL || 'https://andergo.online').replace(/\/$/, ''),
    batchSize: Math.min(100, positiveInteger(process.env.PRACTICE_REMINDER_BATCH_SIZE, 40))
  },
  allowedOrigins
};
