...
const { getLocalLessons, lessonsByLanguage } = require('./lessonsData');
...
const AVAILABLE_LEVELS_BY_LANGUAGE = Object.freeze({
  english: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'],
  spanish: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'],
  french: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'],
  italian: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'],
  portuguese: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'],
  german: ['PRE-A1', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2']
});

function getAvailableLevelsForLanguage(language = 'english') {
  return AVAILABLE_LEVELS_BY_LANGUAGE[language] || AVAILABLE_LEVELS_BY_LANGUAGE.english;
}
...
const unitMetadataByKey = new Map(
  seedUnits.map((unit) => [
    `${unit.target_language}:${unit.level}:${unit.slug}`,
    {
      title: unit.title,
...
function getUnitMetadata(lesson, language) {
  const targetLanguage = lesson.targetLanguage || lesson.language || language;
  return (
    unitMetadataByKey.get(`${targetLanguage}:${lesson.level}:${lesson.unitId}`) ||
...
// Older legacy rows can still have only three Reading choices. When the
// reviewed seed has the same question with four options, use that canonical
// question for delivery and grading without changing unrelated lesson data.
function restoreCanonicalReadingOptions(lesson) {
  if (lesson?.skill !== 'reading') return lesson;
  const language = lesson.targetLanguage || lesson.language;
  const canonical = (lessonsByLanguage[language] || []).find((item) => item.slug === lesson.slug);
  if (!canonical) return lesson;
  let changed = false;
  const exercises = (lesson.exercises || []).map((exercise, index) => {
    const source = canonical.exercises?.[index];
    if (
      exercise?.type !== 'mcq' || exercise?.options?.length !== 3 ||
      source?.type !== 'mcq' || source?.prompt !== exercise.prompt || source?.options?.length !== 4
    ) return exercise;
    changed = true;
    return { ...exercise, options: [...source.options], answer: source.answer };
  });
...
async function fetchFromSupabase(level, language) {
  ...
  return data
    ...
    .map(shapeSupabaseRow)
    .map(restoreCanonicalReadingOptions);
}

async function getLessons({ level = 'A1', language = 'english', userId } = {}) {
  const availableLevels = getAvailableLevelsForLanguage(language);
  ...
  return lessons
    ...
    .map((lesson) => {
      ...
      const unitMetadata = getUnitMetadata(lesson, language);
      const completed = completedSlugs.has(lesson.slug);
      const accessTier = accessPolicy.accessTierForUnit(normalizedLevel, unitOrder, language);
      ...
      if (locked) {
        return {
          ...
          unitTitle: unitMetadata.title,
          unitDescription: unitMetadata.description,
          title: lesson.title,
...
async function findLessonBySlug(slug, language) {
  ...
  if (config.isSupabaseConfigured) {
    ...
    if (data) return restoreCanonicalReadingOptions(shapeSupabaseRow(data));
  }
...
async function completeLesson({
  ...
  // Listening renders its dedicated story question bank instead of the
  // legacy lesson.exercises list. Grade that same (maximum four-question)
  // bank here; otherwise a student can answer every visible question and
  // still be rejected for not attempting hidden legacy exercises.
  const listeningComprehension = lesson.extra?.listeningComprehension;
  const listeningAssessment = listeningComprehension?.questions?.length
    ? {
        ...listeningComprehension,
        questions: listeningComprehension.questions.slice(0, 4)
      }
    : null;
  ...
  const exercisesForAssessment =
    selectedReadingExerciseIds.size > 0
      ? readingExercises.filter((exercise) => selectedReadingExerciseIds.has(String(exercise.id)))
      : readingExercises;
  const scopedAnswers =
    assessmentScope === 'reading_comprehension'
  ...
  const grading = grammarTest?.questions?.length
    ...
    : listeningAssessment?.questions?.length
      ? gradeQuestionBank(listeningAssessment, Array.isArray(answers) ? answers : [])
    : assessmentScope === 'reading_comprehension'
      ? gradeExercises(
...
module.exports = { AVAILABLE_LEVELS_BY_LANGUAGE, getAvailableLevelsForLanguage,
  restoreCanonicalReadingOptions,
  getLessons,
  completeLesson,
  checkAnswer,
