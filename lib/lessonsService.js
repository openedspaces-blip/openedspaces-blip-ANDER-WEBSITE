...
function gradeExercises(realExercises = [], submittedAnswers = []) {
  ...
  realExercises.forEach((exercise, index) => {
    ...
    if (exercise.type === 'mcq') {
      mcqTotal += 1;
      if (!submission || submission.selectedOption === undefined || submission.selectedOption === null) {
        allAttempted = false;
        return;
      }
...
async function completeLesson({ userId, slug, answers = [] }) {
  ...
  const { score, allAttempted } = gradeExercises(lesson.exercises, Array.isArray(answers) ? answers : []);
  if (!allAttempted) {
...
module.exports = { getLessons, completeLesson, checkAnswer, getProgress };
