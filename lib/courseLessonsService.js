...
async function findCourse(languageCode, levelCode) {
  ...
  const { data: level } = await supabase.from('levels').select('id').eq('code', levelCode).maybeSingle();
...
async function loadLessonFull(lessonRow) {
  ...
  const reading = readingSection
    ? { title: readingSection.reading_title, text: readingSection.reading_text, questions: readingSection.reading_questions || [] }
    : null;
  ...
  const exercises = (exerciseRows || []).map((row) => ({
    ...
    type: row.type,
    prompt: row.prompt,
    options: (optionsByExercise[row.id] || []).map((opt) => ({ id: opt.id, text: opt.option_text, isCorrect: opt.is_correct }))
  }));
...
async function getLessons({ languageCode, levelCode, userId }) {
  ...
  const progressMap = await getUserProgressMap(userId, fullLessons.map((l) => l.id));
...
async function startLesson({ userId, slug }) {
  ...
  if (existing) {
    if (existing.status === 'not_started') {
      await supabase
        .from('user_lesson_progress')
        .update({ status: 'in_progress', started_at: existing.started_at || new Date().toISOString() })
        .eq('id', existing.id);
    }
...
async function checkAnswer({ userId, slug, exerciseId, selectedOptionId }) {
  ...
  const supabase = getSupabaseAdmin();
  const { data: exercise } = await supabase.from('exercises').select('*').eq('id', exerciseId).eq('lesson_id', lessonRow.id).maybeSingle();
  if (!exercise) {
  ...
  const { data: options } = await supabase.from('exercise_options').select('*').eq('exercise_id', exerciseId);
  const correctOption = (options || []).find((opt) => opt.is_correct);
  const correct = Boolean(selectedOptionId) && String(selectedOptionId) === String(correctOption?.id);
...
async function gradeExercises(lessonId, submittedAnswers = []) {
  const supabase = getSupabaseAdmin();
  const { data: exerciseRows } = await supabase.from('exercises').select('*').eq('lesson_id', lessonId).order('order_index');
  const exerciseIds = (exerciseRows || []).map((row) => row.id);

  let optionsByExercise = {};
  if (exerciseIds.length) {
    const { data: optionRows } = await supabase.from('exercise_options').select('*').in('exercise_id', exerciseIds);
    optionsByExercise = (optionRows || []).reduce((acc, opt) => {
      (acc[opt.exercise_id] = acc[opt.exercise_id] || []).push(opt);
      return acc;
    }, {});
  ...
  for (const exercise of exerciseRows || []) {
    const submission = submittedAnswers.find((item) => String(item?.exerciseId) === String(exercise.id));
...
async function completeLesson({ userId, slug, answers = [] }) {
  ...
  const { score, allAttempted } = await gradeExercises(lessonRow.id, Array.isArray(answers) ? answers : []);
  if (!allAttempted) {
    const err = new Error('Debes completar todos los ejercicios de la lección antes de terminarla.');
    err.status = 400;
    throw err;
  }
  ...
  const earnedXp = wasAlreadyCompleted ? Math.round((lessonRow.xp_reward || 20) * 0.2) : (lessonRow.xp_reward || 20);
...
module.exports = { hasCourse, hasLesson, getLessons, getLessonDetail, startLesson, checkAnswer, completeLesson, getCompletedCount };
