...
const {
  prepareGrammarTestForPractice,
  sanitizeGrammarTestForClient
} = require('./grammarTestSanitizer');
const accessPolicy = require('./accessPolicyService');

const canonicalReadingExercisesBySlug = new Map(
  seedLessons
    .filter((lesson) => lesson.skill === 'reading' && Array.isArray(lesson.content_json?.exercises))
    .map((lesson) => [lesson.slug, lesson.content_json.exercises])
);

function canonicalReadingOptionId(slug, exerciseIndex, optionIndex) {
  return `canonical-reading-option-${slug}-${exerciseIndex}-${optionIndex}`;
}

function isCanonicalFourthOption({ slug, exerciseIndex, prompt, optionId }) {
  const source = canonicalReadingExercisesBySlug.get(slug)?.[exerciseIndex];
  return (
    source?.type === 'mcq' &&
    source.prompt === prompt &&
    source.options?.length === 4 &&
    String(optionId) === canonicalReadingOptionId(slug, exerciseIndex, 3)
  );
}
...
async function loadLessonFull(lessonRow) {
  ...
  if (exerciseIds.length) {
    const { data: optionRows } = await supabase
      ...
      .order('order_index');
    optionsByExercise = groupOptionsByExercise(optionRows);
  }
  ...
  return assembleLessonFull(lessonRow, sections, exerciseRows, optionsByExercise, dictationRows);
}

function groupOptionsByExercise(optionRows) {
  const optionsByExercise = (optionRows || []).reduce((acc, opt) => {
    (acc[opt.exercise_id] = acc[opt.exercise_id] || []).push(opt);
    return acc;
  }, {});
  Object.values(optionsByExercise).forEach((options) =>
    options.sort((a, b) => (a.order_index || 0) - (b.order_index || 0))
  );
  return optionsByExercise;
}

// Same result as calling loadLessonFull() once per row, but with 4 Supabase
// queries total instead of up to 4 PER LESSON (sections, exercises, options,
// dictation) - getLessons() below used to Promise.all() loadLessonFull()
// across every lesson in a course (72 for a 12-unit course), which meant up
// to ~280 concurrent REST calls to answer one /api/lessons request and was
// measured taking 5+ seconds. Batching via .in(...) keeps the exact same
// per-lesson shape (assembleLessonFull is the same code loadLessonFull uses)
// while cutting that to 4 requests regardless of lesson count.
async function loadLessonsFullBatch(lessonRows) {
  const rows = lessonRows || [];
  if (!rows.length) return [];
  const supabase = getSupabaseAdmin();
  const lessonIds = rows.map((row) => row.id);

  const [{ data: allSections }, { data: allExerciseRows }, { data: allDictationRows }] = await Promise.all([
    supabase.from('lesson_sections').select('*').in('lesson_id', lessonIds).order('order_index'),
    supabase.from('exercises').select('*').in('lesson_id', lessonIds).order('order_index'),
    supabase
      .from('lesson_dictation_segments')
      .select('id, order_index, start_time, end_time, normal_file_path, slow_file_path, very_slow_file_path, lesson_id')
      .in('lesson_id', lessonIds)
      .order('order_index')
  ]);

  const exerciseIds = (allExerciseRows || []).map((row) => row.id);
  let allOptionRows = [];
  if (exerciseIds.length) {
  ...
  const optionsByExercise = groupOptionsByExercise(allOptionRows);
  ...
  const sectionsByLesson = groupBy(allSections, 'lesson_id');
  const exercisesByLesson = groupBy(allExerciseRows, 'lesson_id');
  const dictationByLesson = groupBy(allDictationRows, 'lesson_id');

  return rows.map((lessonRow) =>
    assembleLessonFull(
      lessonRow,
      sectionsByLesson[lessonRow.id],
      exercisesByLesson[lessonRow.id],
      optionsByExercise,
      dictationByLesson[lessonRow.id]
    )
  );
}

// Pure: turns one lesson row's raw related rows into the exact shape the
// client/grading code expects. Shared by loadLessonFull (one lesson, its own
// 4 queries) and loadLessonsFullBatch (many lessons, 4 queries total).
function assembleLessonFull(lessonRow, sections, exerciseRows, optionsByExercise, dictationRows) {
  ...
  const canonicalExercises = canonicalReadingExercisesBySlug.get(lessonRow.slug) || [];
  const exercises = (exerciseRows || []).map((row, index) => {
    const options = (optionsByExercise[row.id] || []).map((opt) => ({
      id: opt.id,
      text: opt.option_text,
      isCorrect: opt.is_correct
    }));
    const canonical = canonicalExercises[index];
    // Historic normalized rows omitted the fourth reviewed choice. A stable
    // synthetic id keeps the existing database option ids intact; grading
    // below resolves that one canonical choice against the source answer key.
    if (
      row.type === 'mcq' &&
      options.length === 3 &&
      canonical?.type === 'mcq' &&
      canonical.prompt === row.prompt &&
      canonical.options?.length === 4
    ) {
      options.push({
        id: canonicalReadingOptionId(lessonRow.slug, index, 3),
        text: canonical.options[3],
        isCorrect: Number(canonical.answer) === 3
      });
    }
    return {
    id: row.id,
  ...
  // (dictationRows is a parameter, already fetched by the caller - see
  // loadLessonFull/loadLessonsFullBatch above.)
  ...
  return {
    ...
    transcript: lessonRow.extra?.transcript || '',
...
async function getLessonAccessContext(lessonRow) {
  ...
  const [{ data: unit }, { data: course }] = await Promise.all([
    supabase
      ...
      .maybeSingle(),
    supabase.from('courses').select('level_id, language_id').eq('id', lessonRow.course_id).maybeSingle()
  ]);
  if (!unit || !course) throw new Error('Lesson access context is incomplete.');
  const [{ data: level }, { data: language }] = await Promise.all([
    supabase.from('levels').select('code').eq('id', course.level_id).maybeSingle(),
    supabase.from('languages').select('code').eq('id', course.language_id).maybeSingle()
  ]);
  if (!level || !language) throw new Error('Lesson language or level is missing.');
  return { level: level.code, language: language.code, unitOrder: unit.order_index, unitId: unit.slug };
}
...
async function getLessons({ languageCode, levelCode, userId }) {
  ...
  const fullLessons = await loadLessonsFullBatch(lessonRows);
  ...
  return fullLessons.map((lesson) => {
    ...
    const accessTier = accessPolicy.accessTierForUnit(levelCode, unit.orderIndex, languageCode);
...
async function getLessonDetail({ slug, userId }) {
  ...
  const accessTier = accessPolicy.accessTierForUnit(context.level, context.unitOrder, context.language);
  if (locked) {
    return {
      slug: lessonRow.slug,
      level: context.level,
...
async function checkAnswer({
  ...
  exerciseIndex,
  selectedOptionIndex,
  // Optional: the caller (server.js's /check-answer route) already has to
  // fetch this row itself via hasLesson()/findLessonRowBySlug() to decide
  // whether this request even belongs to the normalized-schema path at all
  // - passing it through here avoids doing that same lookup a second time
  // on every single answer click, which was adding a full extra network
  // round trip to an already latency-sensitive interaction.
  lessonRow: providedLessonRow
} = {}) {
  const lessonRow = providedLessonRow || (await findLessonRowBySlug(slug));
  if (!lessonRow) {
    const err = new Error('Lesson not found.');
    err.status = 404;
    throw err;
  ...
  if (exerciseId) {
    const { data } = await supabase
      ...
      .maybeSingle();
    exercise = data;
  } else if (Number.isInteger(Number(exerciseIndex)) && Number(exerciseIndex) >= 0) {
    // The bundled world is painted immediately and intentionally contains no
    // database UUIDs or answer keys. Resolve its stable zero-based position
    // against the trusted, ordered database rows instead of rejecting it.
    const { data } = await supabase
      .from('exercises')
      .select('*')
      .eq('lesson_id', lessonRow.id)
      .order('order_index');
    exercise = (data || [])[Number(exerciseIndex)] || null;
  }
  if (!exercise) {
  ...
  // A canonical fourth Reading distractor can be delivered for an older row
  // that did not store it yet. It has no database UUID by design; treat it
  // as an incorrect choice without attempting to save a non-existent foreign
  // key in user_exercise_attempts.
  const suppliedOptionExists = (options || []).some(
    (option) => String(option.id) === String(selectedOptionId)
  );
  const selectedCanonicalFourthOption = isCanonicalFourthOption({
    slug: lessonRow.slug,
    exerciseIndex: Number(exercise.order_index),
    prompt: exercise.prompt,
    optionId: selectedOptionId
  });
  const resolvedSelectedOptionId = suppliedOptionExists
    ? selectedOptionId
    : Number.isInteger(Number(selectedOptionIndex)) && Number(selectedOptionIndex) >= 0
      ? (options || [])[Number(selectedOptionIndex)]?.id || null
      : null;
  const correct =
    selectedCanonicalFourthOption
      ? Number(canonicalReadingExercisesBySlug.get(lessonRow.slug)?.[Number(exercise.order_index)]?.answer) === 3
      : Boolean(resolvedSelectedOptionId) &&
        String(resolvedSelectedOptionId) === String(correctOption?.id);

  if (userId) {
    await supabase.from('user_exercise_attempts').insert({
      user_id: userId,
      exercise_id: exerciseId,
      selected_option_id: resolvedSelectedOptionId || null,
      is_correct: correct
    });
  }
  ...
  return {
    ...
    correctOption: Number.isInteger(correctOption?.order_index)
      ? correctOption.order_index
      : null
  };
}
...
async function checkQuestionBankAnswer({ userId, slug, questionId, answer }) {
  ...
  await assertLessonUnlocked({ userId, lessonRow });
  const bank =
    lessonRow.extra?.grammarTest?.questions?.length
      ? lessonRow.extra.grammarTest
      : lessonRow.extra?.listeningComprehension;
...
function gradeQuestionBank(bank, submittedAnswers = []) {
  const questions = prepareGrammarTestForPractice(bank)?.questions || [];
  let correctCount = 0;
...
async function gradeExercises(
  ...
  if (lessonRow.extra?.listeningComprehension?.questions?.length) {
    return gradeQuestionBank(
      {
        ...lessonRow.extra.listeningComprehension,
        // Keep server grading identical to getListeningComprehensionBank(),
        // which deliberately presents no more than four story questions.
        questions: lessonRow.extra.listeningComprehension.questions.slice(0, 4)
      },
      submittedAnswers
    );
  }
  ...
  let mcqCorrect = 0;
  let allAttempted = true;
  const selectedReadingExerciseIds = new Set(
    assessmentScope === 'reading_comprehension' && [4, 5, 10].includes(assessmentExerciseIds.length)
      ? assessmentExerciseIds.map(String)
      : []
  );

  for (const [exerciseIndex, exercise] of (exerciseRows || []).entries()) {
    const exerciseOptions = optionsByExercise[exercise.id] || [];
    const isTrueFalse =
      exercise.type === 'mcq' &&
    ...
    if (
      ...
      (exercise.type !== 'mcq' || isTrueFalse)
    ) {
      continue;
    }
    if (
      assessmentScope === 'reading_comprehension' &&
      selectedReadingExerciseIds.size > 0 &&
      !selectedReadingExerciseIds.has(String(exercise.id))
    ) {
      continue;
    }
    const submission = submittedAnswers.find((item) =>
      item?.exerciseId
        ? String(item.exerciseId) === String(exercise.id)
        : Number(item?.index) === exerciseIndex
    );

    if (exercise.type === 'mcq') {
      mcqTotal += 1;
      const selectedOptionId = submission?.exerciseId
        ? submission.selectedOptionId
        : exerciseOptions[Number(submission?.selectedOption)]?.id ||
          exerciseOptions[Number(submission?.selectedOptionId)]?.id;
      if (!submission || !selectedOptionId) {
        allAttempted = false;
        continue;
      }
      const correctOption = exerciseOptions.find((opt) => opt.is_correct);
      const selectedCanonicalFourthOption = isCanonicalFourthOption({
        slug: lessonRow.slug,
        exerciseIndex: Number(exercise.order_index),
        prompt: exercise.prompt,
      ...
      const canonicalFourthIsCorrect =
        Number(canonicalReadingExercisesBySlug.get(lessonRow.slug)?.[Number(exercise.order_index)]?.answer) === 3;
      if (
        (selectedCanonicalFourthOption && canonicalFourthIsCorrect) ||
        (correctOption && String(correctOption.id) === String(selectedOptionId))
      ) {
        mcqCorrect += 1;
      }
      continue;
...
async function completeLesson({
  ...
  const { score, allAttempted, results } = await gradeExercises(
    ...
    assessmentScope,
    Array.isArray(assessmentExerciseIds) ? assessmentExerciseIds : []
  );
  if (!allAttempted) {
    const err = new Error(
