...
function isOrderingCorrect(question, submittedOrder) {
  ...
  const submittedIds = (
    Array.isArray(submittedOrder)
      ? submittedOrder
      : Array.isArray(submittedOrder?.itemIds)
        ? submittedOrder.itemIds
        : []
  ).map(String);
  const idsMatch =
    correctOrder.length > 0 &&
    correctOrder.length === submittedIds.length &&
    correctOrder.every((id, index) => id === submittedIds[index]);
  if (idsMatch) return true;

  // A learner can receive the bundled lesson before its remote version has
  // finished synchronizing. In that narrow case the deterministic item ids
  // may differ even though the visible fragments are identical. Compare the
  // displayed fragments only as a compatibility fallback; their order must
  // still match the server's canonical answer exactly.
  const submittedTexts = Array.isArray(submittedOrder?.itemTexts)
    ? submittedOrder.itemTexts.map(normalizeWrittenGrammarAnswer)
    : [];
  const expectedTexts = (question.items || []).map((item) =>
    normalizeWrittenGrammarAnswer(item.text)
  );
  return (
    expectedTexts.length > 0 &&
    expectedTexts.length === submittedTexts.length &&
    expectedTexts.every((text, index) => text === submittedTexts[index])
  );
}
