...
function sanitizeLessonForClient(lesson) {
  return {
    ...
    // extra is otherwise passed through as-is (see loadLessonFull) - its
    // grammarTest key is the one part of it that carries correct answers,
    // so it's the one part that needs stripping here. Same sanitizer used
    // by scripts/sync-worlds-from-seed.js for the offline static bundle,
    // so the two paths can't drift.
    extra: lesson.extra
      ? { ...lesson.extra, grammarTest: sanitizeGrammarTestForClient(lesson.extra.grammarTest) }
      : lesson.extra
  };
}
...
// Re-grades a lesson's grammarTest questions (see
// lib/grammarTestSanitizer.js for the client-facing shape) from the real
// extra.grammarTest.questions - never trusts the client's own claim of
...
function gradeGrammarTest(grammarTest, submittedAnswers = []) {
  const questions = grammarTest.questions || [];
  let correctCount = 0;
  ...
  for (const question of questions) {
    ...
    if (!submission || submission.answer === undefined || submission.answer === null) {
      allAttempted = false;
      results.push({ questionId: question.id, correct: false, explanation: question.explanation || '' });
      continue;
    }
    ...
    results.push({ questionId: question.id, correct, explanation: question.explanation || '' });
  }
...
// has a grammarTest (see lib/grammarTestSanitizer.js), grading happens
// against that question bank instead - the lesson's legacy
// exercises/exercise_options rows, if any, are left untouched/unused so
// nothing else that still reads them (e.g. the printable answer sheet) is
// affected.
async function gradeExercises(lessonRow, submittedAnswers = []) {
  if (lessonRow.extra?.grammarTest?.questions?.length) {
    return gradeGrammarTest(lessonRow.extra.grammarTest, submittedAnswers);
  }
...
async function completeLesson({ userId, slug, answers = [] }) {
  ...
  // best_score/attempts_count, so this is the only place a grammar test's
  // per-attempt history (date, answers, per-question results) survives.
  // Only inserted for lessons that actually have a grammarTest; every
  // other skill/lesson is unaffected.
  if (lessonRow.extra?.grammarTest?.questions?.length) {
