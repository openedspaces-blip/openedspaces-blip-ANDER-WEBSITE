...
// Entitlement + daily-quota gate for the AI Tutor's spoken replies (see
// POST /api/speech/synthesize in lib/server.js). Two independent checks,
...
function isPremiumActive(profile) {
  if (!profile || profile.access_tier !== 'premium') return false;
...
async function checkAndConsumeVoiceQuota({ userId, turnIndex }) {
  ...
  const [{ data: profile }, { data: usage }] = await Promise.all([
    supabase
      .from('profiles')
      .select('access_tier, subscription_status, subscription_expires_at')
      .eq('id', userId)
      .maybeSingle(),
