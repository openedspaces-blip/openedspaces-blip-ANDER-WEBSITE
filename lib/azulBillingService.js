...
const { BUSINESS, formatDopMinorUnits } = require('./businessConfig');
...
function getPublicConfig() {
  return {
    provider: 'azul', configured: missingConfiguration().length === 0, environment: config.azul.environment,
    currencyCode: config.azul.currencyCode || BUSINESS.currencyCode,
    currencySymbol: BUSINESS.currencySymbol,
    plans: {
      monthly: { name: 'ANDERGO Premium mensual', period: 'Mensual', amount: formatDopMinorUnits(config.azul.monthlyAmount) },
      quarterly: { name: 'ANDERGO Premium trimestral', period: 'Cada 3 meses', amount: formatDopMinorUnits(config.azul.quarterlyAmount) }
    }
  };
}
...
async function getReceipt({ userId, orderNumber }) {
  const admin = getSupabaseAdmin();
  if (!admin) throw new Error('Supabase no está configurado.');
  const { data: order, error } = await admin.from('azul_payment_orders')
    .select('order_number,billing_cycle,amount,currency_code,status,authorization_code,processed_at,created_at')
    .eq('order_number', String(orderNumber || '')).eq('user_id', userId).maybeSingle();
  if (error || !order) { const missing = new Error('Comprobante no encontrado.'); missing.status = 404; throw missing; }
  if (order.status !== 'approved') { const unavailable = new Error('El comprobante estará disponible cuando el pago sea aprobado.'); unavailable.status = 409; throw unavailable; }
  return {
    transactionNumber: `AZ-${order.order_number}`, billingCycle: order.billing_cycle,
    service: `ANDERGO Premium ${order.billing_cycle === 'quarterly' ? 'trimestral' : 'mensual'}`,
    subtotal: formatDopMinorUnits(order.amount), taxes: formatDopMinorUnits(config.azul.itbis),
    total: formatDopMinorUnits(order.amount), currency: order.currency_code, status: order.status,
    authorizationCode: order.authorization_code || null, processedAt: order.processed_at || order.created_at,
    business: BUSINESS
  };
}

module.exports = { getPublicConfig, createCheckout, handleReturn, getReceipt, _private: { hash, REQUEST_HASH_FIELDS, RESPONSE_HASH_FIELDS } };
