...
function tutorConfigError() {
  if (!isConfigured()) {
    const err = new Error('El tutor IA no está configurado todavía. Agrega GEMINI_API_KEY en Vercel o en tu .env local.');
    err.status = 503;
    return err;
  }
  return null;
...
function buildTutorInput({
  ...
  return [
    ...
    transcript ? `Transcripción del audio de esta actividad (no confirma que hayas "escuchado" el audio, solo tienes el texto): ${transcript}` : '',
    ...
    isListening ? 'Esta es una actividad de Listening: solo dispones de la transcripción de texto, nunca digas que "escuchaste" el audio como si tuvieras oídos; refiérete a la transcripción o al guion cuando sea relevante.' : '',
    isListening ? 'No reveles la respuesta correcta de inmediato: da pistas progresivas y deja que el estudiante lo intente antes de confirmar o corregir.' : '',
