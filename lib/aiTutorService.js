...
const SPEAKING_CORRECTOR_INSTRUCTIONS = [
  ...
  'Always finish with exactly one separate line in this format: Model: "<target-language model phrase>". The model must preserve the student’s original intention; when their response is already correct, the model may repeat it unchanged.'
].join('\n');
...
const PHONETIC_TRANSCRIPTION_INSTRUCTIONS = [
  ...
  'Return ONLY one valid JSON object matching exactly: {"ipa": string}.',
  'Do not use markdown fences or include commentary outside the JSON object.'
].join('\n');

function parsePhoneticResponse(raw) {
  if (!raw) return null;
...
async function getPhoneticTranscription({ language, text }) {
  ...
  const input = [
    ...
    'Transcribe this text without translating it:',
  ...
  for (const provider of cascade) {
    ...
    try {
      ...
      const parsed = parsePhoneticResponse(buffer);
      if (parsed) return parsed;
