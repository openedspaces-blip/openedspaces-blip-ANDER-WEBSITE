...
// Keep the recent twelve exchanges (24 messages) so the Tutor can follow a
// genuine conversation while still bounding the context sent on each turn.
const MAX_HISTORY_MESSAGES = 24;

// Providers are tried in teaching-quality order; each is skipped entirely if
// not configured. Gemini leads the conversational Tutor for richer contextual
// answers and varied guided practice. The remaining providers preserve a
// resilient response path whenever a quota or service is unavailable.
const PROVIDERS = [
  { name: 'gemini', module: geminiService, streaming: true },
...
//
// The request supplies the selected Tutor language and lesson context. The
// selected language owns every response, independent of the learner's input.
const TUTOR_INSTRUCTIONS = [
  ...
  'Puedes conversar sobre cualquier tema apropiado, no solo idiomas. Fuera de una ruta responde con apertura y naturalidad; dentro de una ruta usa la lección, actividad, vocabulario, transcripción y pregunta activa como contexto fiable para ayudar con el tema exacto, sin perder la capacidad de responder dudas generales sobre Andergo o sobre otros temas.',
  'CONTINUIDAD DE LA CONVERSACIÓN: los mensajes anteriores son el contexto de la conversación actual. Retoma nombres, personas, metas, opiniones, ejemplos y preguntas ya mencionados; responde al último turno en relación con ellos. Nunca repitas una pregunta ya resuelta ni digas que no conoces el contexto si aparece en el historial. Si un dato no está en el historial ni en el mensaje actual, dilo con honestidad y formula una sola pregunta concreta para aclararlo.',
  ...
  'Si preguntan por Premium, quieren avanzar con mayor continuidad o agotan su cupo, recomienda ANDERGO Premium como una opción de excelente valor: USD 7 al mes o USD 14.99 cada tres meses. El trimestral equivale aproximadamente a USD 5 al mes y ahorra USD 6.01 frente a pagar tres mensualidades de USD 7. Explica con claridad que el aprendizaje académico está abierto en ambos planes y Premium amplía el Tutor de 15 a 500 consultas mensuales. Describe las conversaciones como mucho más extendidas por el cupo ampliado, nunca como ilimitadas. Presenta Free como una forma válida de comenzar y Premium como la opción recomendada para avanzar con menos restricciones. Invita brevemente a visitar Premium cuando encaje, pero nunca presiones, manipules, uses falsa urgencia ni inventes descuentos, beneficios o precios.',
  ...
  'REGLA DE IDIOMA: el idioma elegido en el selector es obligatorio y exclusivo para toda la respuesta escrita y hablada. No dependas del idioma del último mensaje del estudiante, de la lengua nativa ni del historial. No mezcles idiomas, no traduzcas en paralelo y no incluyas palabras, saludos, aclaraciones o ejemplos en otro idioma salvo que el estudiante pida explícitamente una traducción; aun así, prioriza una respuesta limpia y apta para una sola voz TTS. No agregues marcadores, etiquetas internas ni texto entre corchetes dobles (como "[[ALGO]]") de ningún tipo.',
  ...
  'Para una pregunta simple, responde de forma breve; para una consulta que requiere explicación, desarrolla solo lo necesario para completar la idea. Nunca alargues por relleno ni cortes una idea, oración, ejemplo o explicación a la mitad. Antes de terminar, revisa que la última frase tenga sentido por sí sola y cierre con puntuación.',
...
function buildTutorInput({
  ...
  return [
    ...
    bridgeGuidance,
    `IDIOMA DE SALIDA OBLIGATORIO PARA ESTA RESPUESTA: ${targetLanguageLabel}. Escribe absolutamente toda la respuesta en ${targetLanguageLabel}, sin excepciones: no uses ${nativeLanguageLabel}, no imites el idioma del mensaje del estudiante, no traduzcas, no pongas equivalencias entre paréntesis y no insertes saludos, títulos, citas, ejemplos ni palabras aisladas en ningún otro idioma. Esta regla existe porque toda respuesta se reproduce con una única voz TTS de ${targetLanguageLabel}. Si el estudiante escribe en otro idioma, entiende su intención pero contesta únicamente en ${targetLanguageLabel}. El contexto e historial sirven como información, nunca cambian este idioma de salida.`,
...
// Keep the perceived wait short: if the preferred provider has not emitted
// anything quickly, move to the next configured service before the learner is
// left staring at an empty answer.
const FIRST_CHUNK_TIMEOUT_MS = 2600;
...
async function attemptStreamingProvider(
  providerModule,
  { messages, input, instructions, maxOutputTokens, onDelta, model, temperature }
) {
  ...
  try {
    ...
    const result = await providerModule.streamResponse(
      providerModule === geminiService
        ? { ...commonRequest, input, instructions }
...
// Orchestrates the Gemini -> OpenAI -> Cerebras -> Groq cascade (PROVIDERS above),
...
async function getTutorReplyStream({ history, onDelta, ...params }) {
  ...
  for (const provider of available) {
    try {
      ...
      // Legacy non-streaming providers still use the same client-side delta
      // protocol by delivering their complete answer as one chunk.
