...
function buildRequestBody({ input, instructions, maxOutputTokens }) {
  return {
    ...
    ...(maxOutputTokens
      ? {
          generationConfig: {
            maxOutputTokens,
            // Conversational tutor replies do not need a reasoning pass before
            // the first visible word. This reduces both latency and cost.
...
function unavailableError() {
  const err = new Error(
    'El Tutor IA está temporalmente ocupado. Inténtalo nuevamente en unos segundos.'
  );
...
async function callGeminiOnce({ input, instructions, model, timeoutMs, maxOutputTokens, signal }) {
  ...
  try {
    res = await fetch(url, {
      ...
      body: JSON.stringify(buildRequestBody({ input, instructions, maxOutputTokens }))
    });
  } catch (error) {
    const aborted = error.name === 'AbortError';
    const err = new Error(
      aborted ? 'Tiempo de espera agotado al contactar Gemini.' : 'No se pudo conectar con Gemini.'
    );
...
// Emits Gemini's text as server-side deltas. The API uses SSE, which lets the
// browser display the first words as soon as Gemini starts generating instead
// of waiting for the entire tutor answer to finish.
async function streamResponse({
  ...
  const url = `${API_BASE}/${encodeURIComponent(model)}:streamGenerateContent?alt=sse&key=${process.env.GEMINI_API_KEY}`;
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
    ...
    const err = new Error(
      error.name === 'AbortError'
        ? 'Tiempo de espera agotado al contactar Gemini.'
  ...
  if (!res.body) {
    const err = new Error('Gemini no abrió un flujo de respuesta.');
    err.status = 502;
    throw err;
  }
  ...
  const emitEvent = (event) => {
    ...
    // Gemini can send either the accumulated candidate or only the newest
    // portion depending on model/version. Normalize both forms to one delta.
  ...
  return { text: emittedText.trim(), model };
}

async function createResponse({ input, instructions, model = DEFAULT_MODEL, maxOutputTokens, signal }) {
  if (!isConfigured()) {
    const err = new Error('GEMINI_API_KEY no está configurado.');
    err.status = 503;
    throw err;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      console.warn(
        `[geminiService] transient error (status ${error.status}), retrying attempt ${attempt + 1}/${MAX_RETRIES}`
      );
...
module.exports = { createResponse, streamResponse, isConfigured, DEFAULT_MODEL };
