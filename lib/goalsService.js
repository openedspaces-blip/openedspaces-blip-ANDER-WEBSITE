...
function shapeGoal(row) {
  if (!row) return null;
  return { id: row.id, goalKey: row.goal_key, selectedAt: row.selected_at, completedAt: row.completed_at ?? null };
}

function shapeDevGoal(goal) {
  if (!goal) return null;
  return { id: goal.id, goalKey: goal.goalKey, selectedAt: goal.selectedAt, completedAt: goal.completedAt ?? null };
}
...
async function upsertGoal(userId, goalKey) {
  ...
  if (config.isSupabaseConfigured) {
    ...
    const { data, error } = await supabase
      .from('user_goals')
      .upsert({ user_id: userId, goal_key: goalKey, selected_at: new Date().toISOString(), completed_at: null }, { onConflict: 'user_id' })
      .select()
      .maybeSingle();
